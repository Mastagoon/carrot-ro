...
struct block_list *battle_gettargeted(struct block_list *target) {
  ...
  map_foreachinallrange(battle_gettargeted_sub, target, AREA_SIZE, BL_CHAR,
                        bl_list, &c, target->id);
  if (c == 0)
    return NULL;
  if (c > 24)
 ...
 * @author [Skotlex]
 */
struct block_list *battle_getenemy(struct block_list *target, int type,
                                   int range) {
  struct block_list *bl_list[24];
  int c = 0;

  memset(bl_list, 0, sizeof(bl_list));
  map_foreachinallrange(battle_getenemy_sub, target, range, type, bl_list, &c,
                        target);
 ...
 * @return Target list
 */
struct block_list *battle_getenemyarea(struct block_list *src, int x, int y,
                                       int range, int type, int ignore_id) {
  struct block_list *bl_list[24];
  int c = 0;

  memset(bl_list, 0, sizeof(bl_list));
  map_foreachinallarea(battle_getenemyarea_sub, src->m, x - range, y - range,
                       x + range, y + range, type, bl_list, &c, src, ignore_id);
...
/*========================================== [Playtester]
 * Deals damage without delay, applies additional effects and triggers monster
 *events This function is called from battle_delay_damage or
 ...
 *(BF_NORMAL|BF_SKILL|BF_SHORT|BF_LONG|BF_WEAPON|BF_MAGIC|BF_MISC)
...
void battle_damage(struct block_list *src, struct block_list *target,
                   int64 damage, t_tick delay, uint16 skill_lv, uint16 skill_id,
                   enum damage_lv dmg_lv, unsigned short attack_type,
                   bool additional_effects, t_tick tick, bool isspdamage) {
  map_freeblock_lock();
  if (isspdamage)
    status_fix_spdamage(src, target, damage, delay, skill_id);
  else
    status_fix_damage(src, target, damage, delay,
                      skill_id); // We have to separate here between reflect
                                 // damage and others [icescope]
  if (attack_type && !status_isdead(target) && additional_effects)
    skill_additional_effect(src, target, skill_id, skill_lv, attack_type,
                            dmg_lv, tick);
  if (dmg_lv > ATK_BLOCK && attack_type)
    skill_counter_additional_effect(src, target, skill_id, skill_lv,
                                    attack_type, tick);
  // This is the last place where we have access to the actual damage type, so
  // any monster events depending on type must be placed here
  if (target->type == BL_MOB) {
    ...
    if (damage && (attack_type &
                   BF_NORMAL)) // Monsters differentiate whether they have been
                               // attacked by a skill or a normal attack
  ...
  if (dat) {
    ...
    if (target && !status_isdead(target)) {
      if (src && target->m == src->m &&
          (target->type != BL_PC ||
           ((TBL_PC *)target)->invincible_timer == INVALID_TIMER) &&
          check_distance_bl(src, target,
                            dat->distance)) // Check to see if you haven't
                                            // teleported. [Skotlex]
      {
        // Deal damage
        battle_damage(src, target, dat->damage, dat->delay, dat->skill_lv,
                      dat->skill_id, dat->dmg_lv, dat->attack_type,
                      dat->additional_effects, tick, dat->isspdamage);
      } else if (!src &&
                 dat->skill_id ==
                     CR_REFLECTSHIELD) { // it was monster reflected damage, and
                                         // the monster died, we pass the damage
                                         // to the character as expected
        map_freeblock_lock();
...
int battle_delay_damage(t_tick tick, int amotion, struct block_list *src,
                        struct block_list *target, int attack_type,
                        uint16 skill_id, uint16 skill_lv, int64 damage,
                        enum damage_lv dmg_lv, t_tick ddelay,
                        bool additional_effects, bool isspdamage) {
  ...
  if (sc) {
    ...
    if (sc->data[SC_WATER_SCREEN_OPTION] &&
        sc->data[SC_WATER_SCREEN_OPTION]->val1)
      e_tbl = map_id2bl(sc->data[SC_WATER_SCREEN_OPTION]->val1);
  }

  if (((d_tbl &&
        check_distance_bl(target, d_tbl, sc->data[SC_DEVOTION]->val3)) ||
       e_tbl) &&
  ...
  if (!battle_config.delay_battle_damage || amotion <= 1) {
    // Deal damage
    battle_damage(src, target, damage, ddelay, skill_lv, skill_id, dmg_lv,
                  attack_type, additional_effects, gettick(), isspdamage);
    return 0;
  }
  ...
  dat->delay = ddelay;
  dat->distance =
      distance_bl(src, target) + (battle_config.snap_dodge ? 10 : AREA_SIZE);
  dat->additional_effects = additional_effects;
  dat->src_type = src->type;
  dat->isspdamage = isspdamage;
  if (src->type != BL_PC && amotion > 1000)
    amotion = 1000; // Aegis places a damage-delay cap of 1 sec to non player
                    // attacks. [Skotlex]
 ...
 * Added passing of the chars so that the status changes can affect it.
 * [Skotlex] Note: Passing src/target == NULL is perfectly valid, it skips SC_
 * checks.
 * @param src
 * @param target
 * @param damage
...
int64 battle_attr_fix(struct block_list *src, struct block_list *target,
  ...
  if (!CHK_ELEMENT(def_type) || !CHK_ELEMENT_LEVEL(def_lv)) {
    ShowError("battle_attr_fix: unknown attribute type: atk=%d def_type=%d "
              "def_lv=%d\n",
  ...
  if (target && target->type == BL_SKILL) {
    if (atk_elem == ELE_FIRE &&
        battle_getcurrentskill(target) == GN_WALLOFTHORN) {
      ...
      if (!su || !su->alive || (sg = su->group) == NULL || !sg ||
          sg->val3 == -1 || (src2 = map_id2bl(sg->src_id)) == NULL ||
          status_isdead(src2))
        return 0;
      ...
      if (sg->unit_id != UNT_FIREWALL) {
        ...
        x = sg->val3 >> 16;
        y = sg->val3 & 0xffff;
        skill_unitsetting(src2, su->group->skill_id, su->group->skill_lv, x, y,
                          1);
        sg->val3 = -1;
  ...
  if (tsc && tsc->count) { // increase dmg by target status
    ...
    case ELE_FIRE:
      ...
      if (tsc->data[SC_THORNSTRAP] &&
          battle_getcurrentskill(src) != GN_CARTCANNON)
    ...
    case ELE_EARTH:
      if (tsc->data[SC_WIND_INSIGNIA])
        ...
        damage += (int64)(damage * 50 / 100);
#endif
      status_change_end(target, SC_MAGNETICFIELD,
                        INVALID_TIMER); // freed if received earth dmg
      break;
    case ELE_NEUTRAL:
...
#ifdef RENEWAL
  // In renewal, reductions are always rounded down so damage can never reach 0
  // unless ratio is 0
 ...
 * @param src Attacker
 * @param target Target
 * @param nk Skill's nk @see enum e_skill_nk
 * [NK_IGNOREATKCARD|NK_IGNOREELEMENT|NK_IGNOREDEFCARD]
 * @param rh_ele Right-hand weapon element
 * @param lh_ele Left-hand weapon element (BF_MAGIC and BF_MISC ignore this
 * value)
 ...
 */
int battle_calc_cardfix(int attack_type, struct block_list *src,
                        struct block_list *target, std::bitset<NK_MAX> nk,
                        int rh_ele, int lh_ele, int64 damage, int left,
                        int flag) {
  ...
  enum e_element
      s_defele; ///< Attacker Element (not a weapon or skill element!)
...
// Official servers apply the cardfix value on a base of 1000 and round down the
// reduction/increase
#define APPLY_CARDFIX(damage, fix)                                             \
  ...
  case BF_MAGIC:
    ...
    if (sd && !nk[NK_IGNOREATKCARD]) {
      ...
      if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
        cardfix = cardfix *
                  ...
                  100;
        cardfix = cardfix *
                  (100 + sd->indexed_bonus.magic_atk_ele[rh_ele] +
                   sd->indexed_bonus.magic_atk_ele[ELE_ALL]) /
                  100;
      }
      cardfix = cardfix *
                (100 + sd->indexed_bonus.magic_addsize[tstatus->size] +
                 sd->indexed_bonus.magic_addsize[SZ_ALL]) /
                100;
      cardfix = cardfix *
    ...
    if (tsd && !nk[NK_IGNOREDEFCARD]) {
      ...
      if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
        int ele_fix = tsd->indexed_bonus.subele[rh_ele] +
        ...
        if (s_defele != ELE_NONE)
          ele_fix += tsd->indexed_bonus.magic_subdefele[s_defele] +
                     tsd->indexed_bonus.magic_subdefele[ELE_ALL];
        cardfix = cardfix * (100 - ele_fix) / 100;
      }
      cardfix = cardfix *
                (100 - tsd->indexed_bonus.subsize[sstatus->size] -
                 tsd->indexed_bonus.subsize[SZ_ALL]) /
                100;
      cardfix = cardfix *
                (100 - tsd->indexed_bonus.magic_subsize[sstatus->size] -
                 tsd->indexed_bonus.magic_subsize[SZ_ALL]) /
                100;
      ...
      race_fix = tsd->indexed_bonus.subrace[sstatus->race] +
                 tsd->indexed_bonus.subrace[RC_ALL];
      ...
      cardfix = cardfix * (100 - race_fix) / 100;
      cardfix = cardfix *
                (100 - tsd->indexed_bonus.subclass[sstatus->class_] -
                 tsd->indexed_bonus.subclass[CLASS_ALL]) /
                100;
      ...
      if (sd->state.arrow_atk) { // Ranged attack
        cardfix = cardfix *
                  (100 + sd->right_weapon.addrace[tstatus->race] +
                   sd->indexed_bonus.arrow_addrace[tstatus->race] +
        ...
        if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
          int ele_fix = sd->right_weapon.addele[tstatus->def_ele] +
                        ...
                        sd->right_weapon.addele[ELE_ALL] +
        ...
        cardfix = cardfix *
                  (100 + sd->right_weapon.addsize[tstatus->size] +
                   sd->indexed_bonus.arrow_addsize[tstatus->size] +
        ...
        cardfix = cardfix *
                  (100 + sd->right_weapon.addclass[tstatus->class_] +
                   ...
                   sd->right_weapon.addclass[CLASS_ALL] +
        ...
        if (!battle_config.left_cardfix_to_right) {
          ...
          if (left & 1) { // Left-handed weapon
            cardfix_ = cardfix_ *
                       (100 + sd->left_weapon.addrace[tstatus->race] +
                        sd->left_weapon.addrace[RC_ALL]) /
                       100;
            if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
            ...
            cardfix_ = cardfix_ *
                       (100 + sd->left_weapon.addsize[tstatus->size] +
                        sd->left_weapon.addsize[SZ_ALL]) /
                       100;
            for (const auto &raceit : t_race2)
              cardfix_ =
                  cardfix_ * (100 + sd->left_weapon.addrace2[raceit]) / 100;
            cardfix_ = cardfix_ *
        ...
        // Calculates right & left hand weapon as unity
          ...
          //! CHECKME: If 'left_cardfix_to_right' is yes, doesn't need to check
          //! NK_IGNOREELEMENT?
          // if( !nk[&]K_IGNOREELEMENT) ) { // Affected by Element modifier
          // bonuses
          int ele_fix = sd->right_weapon.addele[tstatus->def_ele] +
                        sd->left_weapon.addele[tstatus->def_ele] +
                        sd->right_weapon.addele[ELE_ALL] +
                        sd->left_weapon.addele[ELE_ALL];
          ...
          cardfix = cardfix *
                    (100 + sd->right_weapon.addrace[tstatus->race] +
                     sd->left_weapon.addrace[tstatus->race] +
                     sd->right_weapon.addrace[RC_ALL] +
                     sd->left_weapon.addrace[RC_ALL]) /
                    100;
          cardfix = cardfix *
                    (100 + sd->right_weapon.addsize[tstatus->size] +
                     sd->left_weapon.addsize[tstatus->size] +
                     sd->right_weapon.addsize[SZ_ALL] +
                     sd->left_weapon.addsize[SZ_ALL]) /
                    100;
          for (const auto &raceit : t_race2)
            cardfix = cardfix *
                      (100 + sd->right_weapon.addrace2[raceit] +
                       sd->left_weapon.addrace2[raceit]) /
                      100;
          cardfix = cardfix *
                    (100 + sd->right_weapon.addclass[tstatus->class_] +
                     sd->left_weapon.addclass[tstatus->class_] +
                     sd->right_weapon.addclass[CLASS_ALL] +
                     sd->left_weapon.addclass[CLASS_ALL]) /
                    100;
        }
        if (sd->status.weapon == W_KATAR &&
            (skill = pc_checkskill(sd, ASC_KATAR)) >
                0) // Adv. Katar Mastery functions similar to a +%ATK card on
                   // official [helvetica]
          cardfix = cardfix * (100 + (10 + 2 * skill)) / 100;
      }

      //! CHECKME: These right & left hand weapon ignores
    ...
    // Affected by target DEF bonuses
      ...
      if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
        ...
        if (left & 1 && lh_ele != rh_ele) {
          int ele_fix_lh = tsd->indexed_bonus.subele[lh_ele] +
                           tsd->indexed_bonus.subele[ELE_ALL] +
        ...
        cardfix = cardfix *
                  (100 - tsd->indexed_bonus.subdefele[s_defele] -
                   tsd->indexed_bonus.subdefele[ELE_ALL]) /
                  100;
      }
  ...
  case BF_MISC:
    ...
    if (tsd && !nk[NK_IGNOREDEFCARD]) {
      if (!nk[NK_IGNOREELEMENT]) { // Affected by Element modifier bonuses
        ...
        if (s_defele != ELE_NONE)
          ele_fix += tsd->indexed_bonus.subdefele[s_defele] +
                     tsd->indexed_bonus.subdefele[ELE_ALL];
        cardfix = cardfix * (100 - ele_fix) / 100;
      }
      int race_fix = tsd->indexed_bonus.subrace[sstatus->race] +
                     tsd->indexed_bonus.subrace[RC_ALL];
      for (const auto &it : tsd->subrace3) {
        if (it.race != sstatus->race)
          continue;
...
bool battle_status_block_damage(struct block_list *src,
                                struct block_list *target,
                                struct status_change *sc, struct Damage *d,
                                int64 &damage, uint16 skill_id,
                                uint16 skill_lv) {
  if (!src || !target || !sc || !d)
    return true;
  ...
  // ATK_BLOCK Type
  if ((sce = sc->data[SC_SAFETYWALL]) &&
      (flag & (BF_SHORT | BF_MAGIC)) == BF_SHORT) {
  ...
  if ((sc->data[SC_PNEUMA] && (flag & (BF_MAGIC | BF_LONG)) == BF_LONG) ||
      ...
      (sc->data[SC_BASILICA]
       ...
       skill_id != SP_SOULEXPLOSION) ||
      (sc->data[SC_ZEPHYR] && !(flag & BF_MAGIC && skill_id) &&
  ...
  if (sc->data[SC_WHITEIMPRISON]) { // Gravitation and Pressure do damage
                                    // without removing the effect
    if (skill_id == MG_NAPALMBEAT || skill_id == MG_SOULSTRIKE ||
        skill_id == WL_SOULEXPANSION ||
        (skill_id && skill_get_ele(skill_id, skill_lv) == ELE_GHOST) ||
        (skill_id == 0 &&
         (status_get_status_data(src))->rhw.ele == ELE_GHOST)) {
      if (skill_id == WL_SOULEXPANSION)
        damage <<= 1; // If used against a player in White Imprison, the skill
                      // deals double damage.
      status_change_end(
          target, SC_WHITEIMPRISON,
          INVALID_TIMER); // Those skills do damage and removes effect
    } else {
      d->dmg_lv = ATK_BLOCK;
      return false;
    }
  ...
  if ((sce = sc->data[SC_WEAPONBLOCKING]) && flag & (BF_SHORT | BF_WEAPON) &&
      rnd() % 100 < sce->val2) {
    clif_skill_nodamage(target, src, GC_WEAPONBLOCKING, sce->val1, 1);
    sc_start(src, target, SC_WEAPONBLOCK_ON, 100, src->id,
             skill_get_time2(GC_WEAPONBLOCKING, sce->val1));
    d->dmg_lv = ATK_BLOCK;
    return false;
  }
  ...
  if ((sce = sc->data[SC_MILLENNIUMSHIELD]) && sce->val2 > 0 && damage > 0) {
    sce->val3 -=
        static_cast<int>(cap_value(damage, INT_MIN, INT_MAX)); // absorb damage
    d->dmg_lv = ATK_BLOCK;
    if (sce->val3 <= 0) { // Shield Down
      ...
      if (sce->val2 > 0) {
        ...
        status_change_end(target, SC_MILLENNIUMSHIELD,
                          INVALID_TIMER); // All shields down
      status_change_start(src, target, SC_STUN, 10000, 0, 0, 0, 0, 1000,
                          SCSTART_NOTICKDEF);
    }
    return false;
  }
  ...
  if ((sce = sc->data[SC_AUTOGUARD]) && flag & BF_WEAPON &&
    ...
    if (sce_d && (d_bl = map_id2bl(sce_d->val1)) &&
        ...
        check_distance_bl(target, d_bl,
                          sce_d->val3)) { // If player is target of devotion,
                                          // show guard effect on the devotion
                                          // caster rather than the target
      ...
      if (sc->data[SC_SHRINK] && rnd() % 100 < 5 * sce->val1)
        skill_blown(target, src, skill_get_blewcount(CR_SHRINK, 1), -1,
                    BLOWN_NONE);
      d->dmg_lv = ATK_MISS;
      return false;
    }
  ...
  // ATK_DEF Type
  if ((sce = sc->data[SC_LIGHTNINGWALK]) && flag & BF_LONG &&
      rnd() % 100 < sce->val1) {
  ...
  if ((sce = sc->data[SC_PARRYING]) && flag & BF_WEAPON &&
      skill_id != WS_CARTTERMINATION && rnd() % 100 < sce->val2) {
  ...
  if (sc->data[SC_DODGE] && (flag & BF_LONG || sc->data[SC_SPURT]) &&
      (skill_id != NPC_EARTHQUAKE ||
       (skill_id == NPC_EARTHQUAKE && flag & NPC_EARTHQUAKE_FLAG)) &&
      rnd() % 100 < 20) {
    map_session_data *sd = map_id2sd(target->id);
  ...
  if ((sce = sc->data[SC_KAUPE]) &&
      (skill_id != NPC_EARTHQUAKE ||
       (skill_id == NPC_EARTHQUAKE && flag & NPC_EARTHQUAKE_FLAG)) &&
      rnd() % 100 < sce->val2) { // Kaupe blocks damage (skill or otherwise)
                                 // from players, mobs, homuns, mercenaries.
    clif_specialeffect(target, EF_STORMKICK4, AREA);
    // Shouldn't end until Breaker's non-weapon part connects.
#ifndef RENEWAL
    if (skill_id != ASC_BREAKER || !(flag & BF_WEAPON))
#endif
      if (--sce->val3 <= 0) // We make it work like Safety Wall, even though it
                            // only blocks 1 time.
  ...
  if (((sce = sc->data[SC_UTSUSEMI]) || sc->data[SC_BUNSINJYUTSU]) &&
      flag & BF_WEAPON && !skill_get_inf2(skill_id, INF2_IGNORECICADA)) {
    skill_additional_effect(src, target, skill_id, skill_lv, flag, ATK_BLOCK,
                            gettick());
    if (!status_isdead(src))
      skill_counter_additional_effect(src, target, skill_id, skill_lv, flag,
                                      gettick());
    if (sce) {
      clif_specialeffect(target, EF_STORMKICK4, AREA);
 ...
 * @return damage
 */
int64 battle_calc_damage(struct block_list *src, struct block_list *bl,
                         struct Damage *d, int64 damage, uint16 skill_id,
                         uint16 skill_lv) {
  ...
  if (map_getcell(bl->m, bl->x, bl->y, CELL_CHKMAELSTROM) && skill_id &&
      skill_get_type(skill_id) != BF_MISC &&
  ...
  // Nothing can reduce the damage, but Safety Wall and Millennium Shield can
  // block it completely. So can defense sphere's but what the heck is that???
  // [Rytech]
  if (skill_id == SJ_NOVAEXPLOSING &&
      !(sc && (sc->data[SC__MANHOLE] || sc->data[SC_WHITEIMPRISON] ||
               sc->data[SC_GRAVITYCONTROL] || sc->data[SC_KINGS_GRACE] ||
               sc->data[SC_SAFETYWALL] || sc->data[SC_MILLENNIUMSHIELD])))
    return damage;

  if (sc && sc->count) {
    if (!battle_status_block_damage(
            src, bl, sc, d, damage, skill_id,
            skill_lv)) // Statuses that reduce damage to 0.
      return 0;
    ...
    if (sc->data[SC_AETERNA] && skill_id != PF_SOULBURN) {
      if (src->type != BL_MER || !skill_id)
        damage <<= 1; // Lex Aeterna only doubles damage of regular attacks from
                      // mercenaries
      ...
      if (skill_id != ASC_BREAKER || !(flag & BF_WEAPON))
        ...
        status_change_end(bl, SC_AETERNA,
                          INVALID_TIMER); // Shouldn't end until Breaker's
                                          // non-weapon part connects.
    }
    ...
    // Damage reductions
    // Assumptio increases DEF on RE mode, otherwise gives a reduction on the
    // final damage. [Igniz]
#ifndef RENEWAL
    ...
    if (sc->data[SC_DEFENDER] && skill_id != NJ_ZENYNAGE &&
    ...
    if (sc->data[SC_ARMORCHANGE]) {
      // On official servers, SC_ARMORCHANGE does not change DEF/MDEF but rather
      // increases/decreases the damage
      if (flag & BF_WEAPON)
    ...
    if (sc->data[SC_ENERGYCOAT] && (skill_id == GN_HELLS_PLANT_ATK ||
      ...
      int per = 100 * status->sp / status->max_sp -
                1; // 100% should be counted as the 80~99% interval
    ...
    if (damage > 0 && (sce = sc->data[SC_STONEHARDSKIN])) {
      if (src->type ==
          BL_MOB) // using explicit call instead break_equip for duration
        sc_start(src, src, SC_STRIPWEAPON, 30, 0,
                 skill_get_time2(RK_STONEHARDSKIN, sce->val1));
    ...
    // Finally added to remove the status of immobile when Aimed Bolt is used.
    // [Jobbie]
    if (skill_id == RA_AIMEDBOLT && (sc->data[SC_BITE] || sc->data[SC_ANKLE] ||
    ...
    if (sd && (sce = sc->data[SC_FORCEOFVANGUARD]) && flag & BF_WEAPON &&
        rnd() % 100 < sce->val2)
      pc_addspiritball(sd, skill_get_time(LG_FORCEOFVANGUARD, sce->val1),
                       sce->val3);

    if (sd && (sce = sc->data[SC_GT_ENERGYGAIN]) && flag & BF_WEAPON &&
        rnd() % 100 < sce->val2) {
      int spheres = 5;
      ...
      pc_addspiritball(
          sd, skill_get_time2(SR_GENTLETOUCH_ENERGYGAIN, sce->val1), spheres);
    }

    if (sc->data[SC_STYLE_CHANGE] &&
        sc->data[SC_STYLE_CHANGE]->val1 == MH_MD_GRAPPLING) {
      TBL_HOM *hd = BL_CAST(
          BL_HOM, bl); // We add a sphere for when the Homunculus is being hit

      if (hd && (rnd() % 100 <
                 50)) // According to WarpPortal, this is a flat 50% chance
        hom_addspiritball(hd, 10);
    }

    if (sc->data[SC__DEADLYINFECT] &&
        (flag & (BF_SHORT | BF_MAGIC)) == BF_SHORT && damage > 0 &&
  ...
  if (sc && sc->count) {
    ...
    // if ((sce = sc->data[SC_BLOODSUCKER]) && flag & BF_WEAPON && damage > 0 &&
    // rnd() % 100 < (2 * sce->val1 - 1)) status_heal(src, damage * sce->val1 /
    // 100, 0, 3);

    if (flag & BF_MAGIC && bl->type == BL_PC && sc->data[SC_GVG_GIANT] &&
        sc->data[SC_GVG_GIANT]->val4)
    ...
    if (sc->data[SC_POISONINGWEAPON] &&
        ((flag & BF_WEAPON) &&
         (!skill_id ||
          skill_id ==
              GC_VENOMPRESSURE)) // check skill type poison_smoke is a unit
        && (damage > 0 &&
            rnd() % 100 < sc->data[SC_POISONINGWEAPON]
                              ->val3)) // did some damage and chance ok (why no
                                       // additional effect ??)
      sc_start2(src, bl, (enum sc_type)sc->data[SC_POISONINGWEAPON]->val2, 100,
                sc->data[SC_POISONINGWEAPON]->val1,
                sc->data[SC_POISONINGWEAPON]->val4,
                skill_get_time2(GC_POISONINGWEAPON, 1));

    /* Self Buff that destroys the armor of any target hit with melee or ranged
     * physical attacks */
    if (sc->data[SC_SHIELDSPELL_REF] &&
        sc->data[SC_SHIELDSPELL_REF]->val1 == 1 && flag & BF_WEAPON) {
      skill_break_equip(
          src, bl, EQP_ARMOR, 10000,
          BCT_ENEMY); // 100% chance
                      // (http://irowiki.org/wiki/Shield_Spell#Level_3_spells_.28refine_based.29)
    ...
    if (sc->data[SC_STYLE_CHANGE] &&
        sc->data[SC_STYLE_CHANGE]->val1 == MH_MD_FIGHTING) {
      TBL_HOM *hd = BL_CAST(BL_HOM, src); // when attacking

      if (hd && (rnd() % 100 < 50))
        hom_addspiritball(
            hd, 10); // According to WarpPortal, this is a flat 50% chance
    }
  ...
  if (battle_config.pk_mode && sd && bl->type == BL_PC && damage &&
      map_getmapflag(bl->m, MF_PVP)) {
    if (flag & BF_SKILL) { // Skills get a different reduction than non-skills.
                           // [Skotlex]
      if (flag & BF_WEAPON)
 ...
 */
bool battle_can_hit_bg_target(struct block_list *src, struct block_list *bl,
 ...
 */
int64 battle_calc_bg_damage(struct block_list *src, struct block_list *bl,
                            int64 damage, uint16 skill_id, int flag) {
  if (!damage)
    return 0;
  ...
  if (flag &
      BF_SKILL) { // Skills get a different reduction than non-skills. [Skotlex]
    if (flag & BF_WEAPON)
 ...
 * @return Can be hit (true) or can't be hit (false)
 */
bool battle_can_hit_gvg_target(struct block_list *src, struct block_list *bl,
                               uint16 skill_id, int flag) {
  struct mob_data *md = BL_CAST(BL_MOB, bl);
  struct unit_data *ud = unit_bl2ud(bl);
  ...
  if (md && (md->guardian_data || md->special_state.ai == AI_GUILD)) {
    if ((status_bl_has_mode(bl, MD_SKILLIMMUNE) ||
         (class_ == MOBID_EMPERIUM &&
          !skill_get_inf2(skill_id, INF2_TARGETEMPERIUM))) &&
        flag & BF_SKILL) // Skill immunity.
      return false;
    if (src->type != BL_MOB || mob_is_clone(((struct mob_data *)src)->mob_id)) {
      struct guild *g = src->type == BL_PC
                            ? ((TBL_PC *)src)->guild
                            : guild_search(status_get_guild_id(src));

      if (class_ == MOBID_EMPERIUM &&
          (!g || guild_checkskill(g, GD_APPROVAL) <= 0))
        return false;
 ...
 * @return damage
 */
int64 battle_calc_gvg_damage(struct block_list *src, struct block_list *bl,
                             int64 damage, uint16 skill_id, int flag) {
  ...
  if (skill_get_inf2(
          skill_id,
          INF2_IGNOREGVGREDUCTION)) // Skills with no gvg damage reduction.
    return damage;
...
int64 battle_addmastery(struct map_session_data *sd, struct block_list *target,
  ...
  if ((skill = pc_checkskill(sd, AL_DEMONBANE)) > 0 &&
      ...
      (battle_check_undead(status->race, status->def_ele) ||
       status->race == RC_DEMON))
    damage += (skill * (int)(3 + (sd->status.base_level + 1) *
                                     0.05)); // submitted by orn
  if ((skill = pc_checkskill(sd, RA_RANGERMAIN)) > 0 &&
      (status->race == RC_BRUTE || status->race == RC_PLAYER_DORAM ||
  ...
  damage +=
      (15 * pc_checkskill(sd, NC_MADOLICENCE)); // Attack bonus is granted even
                                                // without the Madogear

  if ((skill = pc_checkskill(sd, HT_BEASTBANE)) > 0 &&
      (status->race == RC_INSECT || status->race == RC_BRUTE ||
       status->race == RC_PLAYER_DORAM)) {
    damage += (skill * 4);
  ...
  if (sd->spiritcharm >= MAX_SPIRITCHARM) {
    if ((sd->spiritcharm_type == CHARM_TYPE_FIRE &&
         status->def_ele == ELE_EARTH) ||
        (sd->spiritcharm_type == CHARM_TYPE_WATER &&
         status->def_ele == ELE_FIRE) ||
        (sd->spiritcharm_type == CHARM_TYPE_LAND &&
         status->def_ele == ELE_WIND) ||
        (sd->spiritcharm_type == CHARM_TYPE_WIND &&
         status->def_ele == ELE_WATER))
      damage += damage * 30 / 100;
  }
 ...
 */
static void battle_add_weapon_damage(struct map_session_data *sd, int64 *damage,
                                     int lr_type) {
  if (!sd)
    return;
  // rodatazone says that Overrefine bonuses are part of baseatk
  // Here we also apply the weapon_damage_rate bonus so it is correctly applied
  // on left/right hands.
  if (lr_type == EQI_HAND_L) {
    ...
    if (sd->indexed_bonus.weapon_damage_rate[sd->weapontype2])
      (*damage) += (*damage) *
                   sd->indexed_bonus.weapon_damage_rate[sd->weapontype2] / 100;
    ...
    if (sd->indexed_bonus.weapon_damage_rate[sd->weapontype1])
      (*damage) += (*damage) *
                   sd->indexed_bonus.weapon_damage_rate[sd->weapontype1] / 100;
  }
}

#ifdef RENEWAL
static int battle_calc_sizefix(int64 damage, struct map_session_data *sd,
                               unsigned char t_size, unsigned char weapon_type,
                               short flag) {
  if (sd && !sd->special_state.no_sizefix && !flag) // Size fix only for players
    damage = damage *
             (weapon_type == EQI_HAND_L ? sd->left_weapon.atkmods[t_size]
...
/**
 * Calculates renewal Variance, OverUpgradeBonus, and SizePenaltyMultiplier of
 * weapon damage parts for player
 ...
 */
static int battle_calc_base_weapon_attack(struct block_list *src,
  ...
  if (sd && sd->equip_index[type] >= 0 &&
      sd->inventory_data[sd->equip_index[type]]) {
    short base_stat;
  ...
  damage =
      battle_calc_sizefix(damage, sd, tstatus->size, type, weapon_perfection);
 ...
 * it calculates nothing extra fancy, is needed for magnum break's WATK_ELEMENT
 *bonus. [Skotlex] This applies to pre-renewal and non-sd in renewal
 *------------------------------------------
 ...
 * &16: Arrow attack but BOW, REVOLVER, RIFLE, SHOTGUN, GATLING or GRENADE type
 *weapon not equipped (i.e. shuriken, kunai and venom knives not affected by
 *DEX)
 *
 * Credits:
 *	Original coder Skotlex
...
static int64 battle_calc_base_damage(struct block_list *src,
                                     ...
                                     struct weapon_atk *wa,
                                     struct status_change *sc,
                                     unsigned short t_size, int flag) {
  ...
  if (!sd) { // Mobs/Pets
    ...
    if (!(flag & 1) || (flag & 2)) { // Normal attacks
      ...
      if (sd->equip_index[type] >= 0 &&
          sd->inventory_data[sd->equip_index[type]])
        atkmin = atkmin *
                 (80 + sd->inventory_data[sd->equip_index[type]]->wlv * 20) /
                 100;
  ...
  if (sd) {
    ...
    if (flag & 2 && sd->bonus.arrow_atk)
      damage +=
          ((flag & 1) ? sd->bonus.arrow_atk : rnd() % sd->bonus.arrow_atk);

    // Size fix only for players
    if (!(sd->special_state.no_sizefix || (flag & 8)))
      damage = damage *
               (type == EQI_HAND_L ? sd->left_weapon.atkmods[t_size]
                                   : sd->right_weapon.atkmods[t_size]) /
               100;
    ...
    if (ele_sc) {
      ...
      case ELEMENTALID_AGNI_L:
        if (ele_sc->data[SC_FIRE_INSIGNIA] &&
            ele_sc->data[SC_FIRE_INSIGNIA]->val1 == 1)
          damage += damage * 20 / 100;
        break;
      case ELEMENTALID_AQUA_S:
      case ELEMENTALID_AQUA_M:
      case ELEMENTALID_AQUA_L:
        if (ele_sc->data[SC_WATER_INSIGNIA] &&
            ele_sc->data[SC_WATER_INSIGNIA]->val1 == 1)
          damage += damage * 20 / 100;
        break;
      case ELEMENTALID_VENTUS_S:
      case ELEMENTALID_VENTUS_M:
      case ELEMENTALID_VENTUS_L:
        if (ele_sc->data[SC_WIND_INSIGNIA] &&
            ele_sc->data[SC_WIND_INSIGNIA]->val1 == 1)
          damage += damage * 20 / 100;
        break;
      case ELEMENTALID_TERA_S:
      case ELEMENTALID_TERA_M:
      case ELEMENTALID_TERA_L:
        if (ele_sc->data[SC_EARTH_INSIGNIA] &&
            ele_sc->data[SC_EARTH_INSIGNIA]->val1 == 1)
          damage += damage * 20 / 100;
        break;
      }
    }
...
void battle_consume_ammo(struct map_session_data *sd, int skill, int lv) {
  ...
  if (sd->equip_index[EQI_AMMO] >=
      0) // Qty check should have been done in skill_check_condition
    pc_delitem(sd, sd->equip_index[EQI_AMMO], qty, 0, 1, LOG_TYPE_CONSUME);

  sd->state.arrow_atk = 0;
}

static int battle_range_type(struct block_list *src, struct block_list *target,
  ...
  case GC_CROSSIMPACT:
    // Cast range is 7 cells and player jumps to target but skill is considered
    // melee
    return BF_SHORT;
  }
  ...
  if (battle_config.skillrange_by_distance &&
      (src->type &
       battle_config.skillrange_by_distance)) { // based on distance between
                                                // src/target [Skotlex]
...
static int battle_blewcount_bonus(struct map_session_data *sd,
                                  uint16 skill_id) {
...
static enum e_skill_damage_type
battle_skill_damage_type(struct block_list *bl) {
  switch (bl->type) {
  case BL_PC:
    return SKILLDMG_PC;
  case BL_MOB:
 ...
 */
static int battle_skill_damage_skill(struct block_list *src,
                                     struct block_list *target,
                                     uint16 skill_id) {
  ...
  if ((damage->map & 1 &&
       (!mapdata->flag[MF_PVP] && !mapdata_flag_gvg2(mapdata) &&
        !mapdata->flag[MF_BATTLEGROUND] && !mapdata->flag[MF_SKILL_DAMAGE] &&
        !mapdata->flag[MF_RESTRICTED])) ||
 ...
 * @return Skill damage rate
 */
static int battle_skill_damage_map(struct block_list *src,
                                   struct block_list *target, uint16 skill_id) {
  map_data *mapdata = map_getmapdata(src->m);
  ...
  if (mapdata->skill_damage.find(skill_id) != mapdata->skill_damage.end() &&
    ...
    rate +=
        mapdata->skill_damage[skill_id].rate[battle_skill_damage_type(target)];
  }
  return rate;
}

/**
 * Check skill damage adjustment based on mapflags and skill_damage_db.txt for
 * specified skill
 * @param src
 * @param target
 * @param skill_id
 ...
 */
static int battle_skill_damage(struct block_list *src,
                               struct block_list *target, uint16 skill_id) {
  nullpo_ret(src);
  if (!target || !skill_id)
    return 0;
  skill_id = skill_dummy2skill_id(skill_id);
  return battle_skill_damage_skill(src, target, skill_id) +
         battle_skill_damage_map(src, target, skill_id);
}

static inline int
battle_calc_damage_adjustment_sub(struct s_global_damage_rate *rates,
                                  bool use_mapflag, int flag) {
  if (flag & BF_SKILL) {
    if (flag & BF_WEAPON)
      return use_mapflag ? rates->rate[DMGRATE_WEAPON]
                         : battle_config.atk_weapon_damage_rate;
    if (flag & BF_MAGIC)
      return use_mapflag ? rates->rate[DMGRATE_MAGIC]
                         : battle_config.atk_magic_damage_rate;
    if (flag & BF_MISC)
      return use_mapflag ? rates->rate[DMGRATE_MISC]
                         : battle_config.atk_misc_damage_rate;
  } else {
    if (flag & BF_SHORT)
      return use_mapflag ? rates->rate[DMGRATE_SHORT]
                         : battle_config.atk_short_damage_rate;
    if (flag & BF_LONG)
      return use_mapflag ? rates->rate[DMGRATE_LONG]
 ...
 * @param damage
 **/
static int64 battle_calc_damage_adjustment(struct block_list *src, int64 damage,
                                           int flag) {
  ...
  attacker = map_atk_rate ? mapd->atk_rate.rate[DMGRATE_BL]
                          : battle_config.atk_damage_attacker;
  ...
  // Checking mapflag
  if ((atk_maps & 1 &&
       (!map_pvp && !map_gvg && !map_bg && !map_atk_rate && !mapd->zone)) ||
      (atk_maps & 2 && map_pvp) || (atk_maps & 4 && map_gvg) ||
      (atk_maps & 8 && map_bg) || (atk_maps & 16 && map_atk_rate) ||
      (mapd->zone && atk_maps & (8 * mapd->zone))) {
    rate =
        battle_calc_damage_adjustment_sub(&mapd->atk_rate, map_atk_rate, flag);
  }
...
int battle_calc_chorusbonus(struct map_session_data *sd) {
  ...
  members = party_foreachsamemap(party_sub_count_class, sd, 0, MAPID_THIRDMASK,
                                 MAPID_MINSTRELWANDERER);

  if (members < 3)
    return 0; // Bonus remains 0 unless 3 or more Minstrels/Wanderers are in the
              // party.
  if (members > 7)
    return 5; // Maximum effect possible from 7 or more Minstrels/Wanderers.
  return members - 2; // Effect bonus from additional Minstrels/Wanderers if not
                      // above the max possible.
}

struct Damage battle_calc_magic_attack(struct block_list *src,
                                       struct block_list *target,
                                       uint16 skill_id, uint16 skill_lv,
                                       int mflag);
struct Damage battle_calc_misc_attack(struct block_list *src,
                                      struct block_list *target,
                                      uint16 skill_id, uint16 skill_lv,
                                      int mflag);
...
bool is_infinite_defense(struct block_list *target, int flag) {
  ...
  if (target->type == BL_SKILL) {
    ...
    if (su && su->group &&
        (su->group->skill_id == NPC_REVERBERATION ||
         su->group->skill_id == WM_POEMOFNETHERWORLD))
      return true;
  }

  if (status_has_mode(tstatus, MD_IGNOREMELEE) &&
      (flag & (BF_WEAPON | BF_SHORT)) == (BF_WEAPON | BF_SHORT))
    return true;
  if (status_has_mode(tstatus, MD_IGNOREMAGIC) && flag & (BF_MAGIC))
    return true;
  if (status_has_mode(tstatus, MD_IGNORERANGED) &&
      (flag & (BF_WEAPON | BF_LONG)) == (BF_WEAPON | BF_LONG))
    return true;
...
static bool is_skill_using_arrow(struct block_list *src, int skill_id) {
  if (src != NULL) {
    ...
    return ((sd && sd->state.arrow_atk) ||
            (!sd && ((skill_id && skill_get_ammotype(skill_id)) ||
                     sstatus->rhw.range > 3)) ||
...
static bool is_attack_right_handed(struct block_list *src, int skill_id) {
  if (src != NULL) {
    ...
    if (!skill_id && sd && sd->weapontype1 == W_FIST &&
 ...
 *	Refined and optimized by helvetica
 */
static bool is_attack_critical(struct Damage *wd, struct block_list *src,
                               struct block_list *target, int skill_id,
                               int skill_lv, bool first_call) {
  if (!first_call)
    return (wd->type == DMG_CRITICAL || wd->type == DMG_MULTI_HIT_CRITICAL);

#ifdef RENEWAL
  if (skill_id == NPC_CRITICALSLASH ||
      skill_id == LG_PINPOINTATTACK) // Always critical skills
    return true;
#endif
  ...
  if (sstatus->cri) {
    ...
    if (wd->type == DMG_MULTI_HIT) { // Multiple Hit Attack Skills.
      if (pc_checkskill(sd, GS_CHAINACTION) &&
          !skill_get_nk(GS_CHAINACTION, NK_CRITICAL)) // Chain Action
        return false;

      if (pc_checkskill(sd, TF_DOUBLE) &&
          !skill_get_nk(TF_DOUBLE, NK_CRITICAL)) // Double Attack
        return false;
    }
    ...
    if (sd) {
      cri += sd->indexed_bonus.critaddrace[tstatus->race] +
             sd->indexed_bonus.critaddrace[RC_ALL];
    ...
    // Therefore, we use the old value 3 on cases when an sd gets attacked by a
    // mob
 ...
 *	Refined and optimized by helvetica
 */
static int is_attack_piercing(struct Damage *wd, struct block_list *src,
                              struct block_list *target, int skill_id,
                              int skill_lv, short weapon_position) {
  ...
  if (src != NULL) {
    ...
    if (skill_id != PA_SACRIFICE && skill_id != CR_GRANDCROSS &&
        skill_id != NPC_GRANDDARKNESS && skill_id != PA_SHIELDCHAIN &&
      ...
      if (sd &&
          (sd->right_weapon.def_ratio_atk_ele & (1 << tstatus->def_ele) ||
      ...
      if (sd && (sd->left_weapon.def_ratio_atk_ele & (1 << tstatus->def_ele) ||
                 ...
                 sd->left_weapon.def_ratio_atk_class &
                     (1 << CLASS_ALL))) { // Pass effect onto right hand if
                                          // configured so. [Skotlex]
        if (battle_config.left_cardfix_to_right &&
            is_attack_right_handed(src, skill_id)) {
          if (weapon_position == EQI_HAND_R)
            return 1;
...
static std::bitset<NK_MAX> battle_skill_get_damage_properties(uint16 skill_id,
                                                              int is_splash) {
  if (skill_id == 0) {
    if (is_splash) {
 ...
 *	Refined and optimized by helvetica
 */
static bool is_attack_hitting(struct Damage *wd, struct block_list *src,
                              struct block_list *target, int skill_id,
                              int skill_lv, bool first_call) {
  struct status_data *sstatus = status_get_status_data(src);
  struct status_data *tstatus = status_get_status_data(target);
  ...
  std::bitset<NK_MAX> nk =
      battle_skill_get_damage_properties(skill_id, wd->miscflag);
  short flee, hitrate;
  ...
  if (is_attack_critical(wd, src, target, skill_id, skill_lv, false))
    return true;
  else if (sd && sd->bonus.perfect_hit > 0 &&
           rnd() % 100 < sd->bonus.perfect_hit)
    return true;
    ...
    return true;
           ...
           !wd->miscflag)
    return true;
  else if (skill_id == CR_SHIELDBOOMERANG && sc && sc->data[SC_SPIRIT] &&
           sc->data[SC_SPIRIT]->val2 == SL_CRUSADER)
    return true;
  else if (tsc && tsc->opt1 && tsc->opt1 != OPT1_STONEWAIT &&
           tsc->opt1 != OPT1_BURNING)
    return true;
  else if (nk[NK_IGNOREFLEE])
    return true;

  if (tsc && tsc->data[SC_NEUTRALBARRIER] &&
      (wd->flag & (BF_LONG | BF_MAGIC)) == BF_LONG)
    return false;
  ...
  if (battle_config.agi_penalty_type &&
    ...
    unsigned char attacker_count =
        unit_counttargeted(target); // 256 max targets should be a sane max

    if (attacker_count >= battle_config.agi_penalty_count) {
      if (battle_config.agi_penalty_type == 1)
        flee =
            (flee *
             (100 - (attacker_count - (battle_config.agi_penalty_count - 1)) *
                        battle_config.agi_penalty_num)) /
            100;
      else // assume type 2: absolute reduction
        flee -= (attacker_count - (battle_config.agi_penalty_count - 1)) *
                battle_config.agi_penalty_num;
      if (flee < 1)
        flee = 1;
    }
  ...
  if ((wd->flag & (BF_LONG | BF_MAGIC)) == BF_LONG && !skill_id && tsc &&
      tsc->data[SC_FOGWALL])
    hitrate -= 50;
...
#ifdef RENEWAL
  if (sd) // in Renewal hit bonus from Vultures Eye is not anymore shown in
          // status window
  ...
  if (skill_id) {
...
#endif
    // case RK_SONICWAVE:
    // 	hitrate += hitrate * 3 * skill_lv / 100; // !TODO: Confirm the hitrate
    // bonus 	break;
    ...
    case RL_SLUGSHOT: {
      ...
      if (dist > 3) {
        // Reduce n hitrate for each cell after initial 3 cells. Different each
        // level -10:-9:-8:-7:-6
        dist -= 3;
        hitrate -= ((11 - skill_lv) * dist);
             ...
             wd->div_ == 2) // +1 hit per level of Double Attack on a successful
                            // double attack (making sure other multi attack
                            // skills do not trigger this) [helvetica]
  ...
  hitrate =
      cap_value(hitrate, battle_config.min_hitrate, battle_config.max_hitrate);
 ...
 *	Refined and optimized by helvetica
 */
static bool attack_ignores_def(struct Damage *wd, struct block_list *src,
                               struct block_list *target, uint16 skill_id,
                               uint16 skill_lv, short weapon_position) {
  struct status_data *tstatus = status_get_status_data(target);
  struct status_change *sc = status_get_sc(src);
  struct map_session_data *sd = BL_CAST(BL_PC, src);
  std::bitset<NK_MAX> nk =
  ...
  if (is_attack_critical(wd, src, target, skill_id, skill_lv, false))
    ...
    return true;
  else if (skill_id != CR_GRANDCROSS &&
           skill_id != NPC_GRANDDARKNESS) { // Ignore Defense?
    ...
    if (sd && (sd->left_weapon.ignore_def_ele & (1 << tstatus->def_ele) ||
      ...
      if (battle_config.left_cardfix_to_right &&
          is_attack_right_handed(
              src, skill_id)) { // Move effect to right hand. [Skotlex]
        if (weapon_position == EQI_HAND_R)
          return true;
      } else if (weapon_position == EQI_HAND_L)
        return true;
...
static bool battle_skill_stacks_masteries_vvs(uint16 skill_id) {
  if (
      ...
      skill_id == NC_SELFDESTRUCTION || skill_id == LG_SHIELDPRESS ||
 ...
 *	Refined and optimized by helvetica
 */
static int battle_get_weapon_element(struct Damage *wd, struct block_list *src,
                                     struct block_list *target, uint16 skill_id,
                                     uint16 skill_lv, short weapon_position,
  ...
  if (!skill_id || element == ELE_WEAPON) {
    ...
    if (is_skill_using_arrow(src, skill_id) && sd && sd->bonus.arrow_ele &&
        weapon_position == EQI_HAND_R)
      element = sd->bonus.arrow_ele;
    if (sd && sd->spiritcharm_type != CHARM_TYPE_NONE &&
        sd->spiritcharm >= MAX_SPIRITCHARM)
      element = sd->spiritcharm_type; // Summoning 10 spiritcharm will endow
                                      // your weapon
  ...
  case SJ_PROMINENCEKICK:
    ...
    break;
  case RL_H_MINE:
    if (sd && sd->flicker) // Force RL_H_MINE deals fire damage if activated by
                           // RL_FLICKER
      element = ELE_FIRE;
    break;
  }
  ...
  if (sc && sc->data[SC_GOLDENE_FERSE] &&
      ((!skill_id && (rnd() % 100 < sc->data[SC_GOLDENE_FERSE]->val4)) ||
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_element_damage(struct Damage *wd,
  ...
  if (!nk[NK_IGNOREELEMENT] && (wd->damage > 0 || wd->damage2 > 0)) {
    ...
    struct status_data *sstatus = status_get_status_data(src);
    struct status_data *tstatus = status_get_status_data(target);
    int left_element = battle_get_weapon_element(wd, src, target, skill_id,
                                                 skill_lv, EQI_HAND_L, true);
    int right_element = battle_get_weapon_element(wd, src, target, skill_id,
                                                  skill_lv, EQI_HAND_R, true);
    ...
    case HFLI_SBR44:
      wd->damage = battle_attr_fix(src, target, wd->damage, right_element,
                                   tstatus->def_ele, tstatus->ele_lv);
      if (is_attack_left_handed(src, skill_id))
        wd->damage2 = battle_attr_fix(src, target, wd->damage2, left_element,
                                      tstatus->def_ele, tstatus->ele_lv);
      break;
    default:
      if (skill_id == 0 && (battle_config.attack_attr_none & src->type))
        return; // Non-player basic attacks are non-elemental, they deal 100%
                // against all defense elements
#ifdef RENEWAL
      if (sd == nullptr) { // Renewal player's elemental damage calculation is
                           // already done before this point, only calculate for
                           // everything else
#endif
        wd->damage = battle_attr_fix(src, target, wd->damage, right_element,
                                     tstatus->def_ele, tstatus->ele_lv);
    ...
    // Skills forced to neutral gain benefits from weapon element but final
    // damage is considered "neutral" and resistances are applied again
    switch (skill_id) {
#ifdef RENEWAL
    case MO_INVESTIGATE:
    ...
    case GN_FIRE_EXPANSION_ACID:
      wd->damage = battle_attr_fix(src, target, wd->damage, ELE_NEUTRAL,
                                   tstatus->def_ele, tstatus->ele_lv);
      if (is_attack_left_handed(src, skill_id))
        wd->damage2 = battle_attr_fix(src, target, wd->damage2, ELE_NEUTRAL,
                                      tstatus->def_ele, tstatus->ele_lv);
      break;
    }

#ifdef RENEWAL
    if (sd == nullptr) { // Only monsters have a single ATK for element, in
                         // pre-renewal we also apply element to entire ATK on
                         // players [helvetica]
#endif
      if (sc &&
          sc->data[SC_WATK_ELEMENT]) { // Descriptions indicate this means
                                       // adding a percent of a normal attack in
                                       // another element [Skotlex]
        int64 damage = battle_calc_base_damage(
                           src, sstatus, &sstatus->rhw, sc, tstatus->size,
                           (is_skill_using_arrow(src, skill_id) ? 2 : 0)) *
                       sc->data[SC_WATK_ELEMENT]->val2 / 100;

        wd->damage += battle_attr_fix(src, target, damage,
                                      sc->data[SC_WATK_ELEMENT]->val1,
                                      tstatus->def_ele, tstatus->ele_lv);
        if (is_attack_left_handed(src, skill_id)) {
          damage = battle_calc_base_damage(
                       src, sstatus, &sstatus->lhw, sc, tstatus->size,
                       (is_skill_using_arrow(src, skill_id) ? 2 : 0)) *
                   sc->data[SC_WATK_ELEMENT]->val2 / 100;
          wd->damage2 += battle_attr_fix(src, target, damage,
                                         sc->data[SC_WATK_ELEMENT]->val1,
                                         tstatus->def_ele, tstatus->ele_lv);
        }
      }
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_masteries(struct Damage *wd,
                                         struct block_list *src,
                                         struct block_list *target,
                                         uint16 skill_id, uint16 skill_lv) {
  struct map_session_data *sd = BL_CAST(BL_PC, src);
  ...
  if (sd && battle_skill_stacks_masteries_vvs(skill_id) &&
    ...
    if (skill_id == TF_POISON) // Additional ATK from Envenom is treated as
                               // mastery type damage [helvetica]
    ...
    if (skill_id != CR_SHIELDBOOMERANG)
      ATK_ADD2(wd->masteryAtk, wd->masteryAtk2,
               ((wd->div_ < 1) ? 1 : wd->div_) * sd->right_weapon.star,
               ((wd->div_ < 1) ? 1 : wd->div_) * sd->left_weapon.star);
    if (skill_id == MO_FINGEROFFENSIVE) {
      ATK_ADD(wd->masteryAtk, wd->masteryAtk2,
              ((wd->div_ < 1) ? 1 : wd->div_) * sd->spiritball_old * 3);
    } else
      ATK_ADD(wd->masteryAtk, wd->masteryAtk2,
              ((wd->div_ < 1) ? 1 : wd->div_) * sd->spiritball * 3);
#endif

    if (skill_id == NJ_SYURIKEN &&
        (skill = pc_checkskill(sd, NJ_TOBIDOUGU)) >
            0) { // !TODO: Confirm new mastery formula
    ...
    if (sc) { // Status change considered as masteries
#ifdef RENEWAL
      if (sc->data[SC_NIBELUNGEN]) // With renewal, the level 4 weapon
      ...
      if (sc->data[SC_CAMOUFLAGE]) {
        ATK_ADD(wd->damage, wd->damage2,
                30 * min(10, sc->data[SC_CAMOUFLAGE]->val3));
#ifdef RENEWAL
        ATK_ADD(wd->masteryAtk, wd->masteryAtk2,
                30 * min(10, sc->data[SC_CAMOUFLAGE]->val3));
#endif
...
#ifdef RENEWAL
        ATK_ADD(wd->masteryAtk, wd->masteryAtk2,
                10 * sc->data[SC_GN_CARTBOOST]->val1);
#endif
      }
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_damage_parts(struct Damage *wd, struct block_list *src,
                                     struct block_list *target, uint16 skill_id,
                                     uint16 skill_lv) {
  struct status_data *sstatus = status_get_status_data(src);
  struct status_data *tstatus = status_get_status_data(target);
  ...
  int right_element = battle_get_weapon_element(wd, src, target, skill_id,
                                                skill_lv, EQI_HAND_R, false);
  int left_element = battle_get_weapon_element(wd, src, target, skill_id,
                                               skill_lv, EQI_HAND_L, false);

  wd->statusAtk += battle_calc_status_attack(sstatus, EQI_HAND_R);
  wd->statusAtk2 += battle_calc_status_attack(sstatus, EQI_HAND_L);

  if (sd &&
      sd->sc.data[SC_SEVENWIND]) { // Mild Wind applies element to status ATK as
                                   // well as weapon ATK [helvetica]
    wd->statusAtk = battle_attr_fix(src, target, wd->statusAtk, right_element,
                                    tstatus->def_ele, tstatus->ele_lv);
    wd->statusAtk2 = battle_attr_fix(src, target, wd->statusAtk, left_element,
                                     tstatus->def_ele, tstatus->ele_lv);
  } else { // status atk is considered neutral on normal attacks [helvetica]
    wd->statusAtk = battle_attr_fix(src, target, wd->statusAtk, ELE_NEUTRAL,
                                    tstatus->def_ele, tstatus->ele_lv);
    wd->statusAtk2 = battle_attr_fix(src, target, wd->statusAtk, ELE_NEUTRAL,
                                     tstatus->def_ele, tstatus->ele_lv);
  }

  wd->weaponAtk +=
      battle_calc_base_weapon_attack(src, tstatus, &sstatus->rhw, sd);
  wd->weaponAtk = battle_attr_fix(src, target, wd->weaponAtk, right_element,
                                  tstatus->def_ele, tstatus->ele_lv);

  wd->weaponAtk2 +=
      battle_calc_base_weapon_attack(src, tstatus, &sstatus->lhw, sd);
  wd->weaponAtk2 = battle_attr_fix(src, target, wd->weaponAtk2, left_element,
                                   tstatus->def_ele, tstatus->ele_lv);

  wd->equipAtk += battle_calc_equip_attack(src, skill_id);
  wd->equipAtk = battle_attr_fix(src, target, wd->equipAtk, right_element,
                                 tstatus->def_ele, tstatus->ele_lv);

  wd->equipAtk2 += battle_calc_equip_attack(src, skill_id);
  wd->equipAtk2 = battle_attr_fix(src, target, wd->equipAtk2, left_element,
                                  tstatus->def_ele, tstatus->ele_lv);

  // Mastery ATK is a special kind of ATK that has no elemental properties
  // Because masteries are not elemental, they are unaffected by Ghost armors or
  // Raydric Card
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_skill_base_damage(struct Damage *wd,
                                          struct block_list *src,
                                          struct block_list *target,
                                          uint16 skill_id, uint16 skill_lv) {
  struct status_change *sc = status_get_sc(src);
    ...
    if (sd) {
      ...
      if (index >= 0 && sd->inventory_data[index] &&
        ...
        wd->equipAtk += sd->inventory_data[index]->weight * 7 /
                        100; // weight from spear is treated as equipment ATK on
                             // official [helvetica]
      ...
      switch (
          tstatus->size) { // Size-fix. Is this modified by weapon perfection?
      ...
      wd->damage = battle_calc_base_damage(
          src, sstatus, &sstatus->rhw, sc, tstatus->size,
          0); // Monsters have no weight and use ATK instead
    }
#else
  case NJ_ISSEN:
  ...
  case ML_SPIRALPIERCE:
    if (sd) {
      ...
      if (index >= 0 && sd->inventory_data[index] &&
          sd->inventory_data[index]->type == IT_WEAPON)
        wd->damage =
            sd->inventory_data[index]->weight * 8 / 100; // 80% of weight

      ATK_ADDRATE(wd->damage, wd->damage2,
                  50 * skill_lv); // Skill modifier applies to weight only.
    } else {
      wd->damage = battle_calc_base_damage(
          src, sstatus, &sstatus->rhw, sc, tstatus->size,
          0); // Monsters have no weight and use ATK instead
...
#ifdef RENEWAL
        ATK_ADD(wd->weaponAtk, wd->weaponAtk2,
                sd->inventory_data[index]->weight / 10);
#endif
      }
    } else
  ...
  case RK_DRAGONBREATH_WATER: {
    int damagevalue =
        (sstatus->hp / 50 + status_get_max_sp(src) / 4) * skill_lv;

    if (status_get_lv(src) > 100)
      damagevalue = damagevalue *
                    (100 + 5 * (pc_checkskill(sd, RK_DRAGONTRAINING) - 1)) /
                    100;
    if (sd)
      damagevalue =
  ...
  case NC_SELFDESTRUCTION: {
    int damagevalue = (skill_lv + 1) *
                      ((sd ? pc_checkskill(sd, NC_MAINFRAME) : 0) + 8) *
      ...
      int damagevalue = 3 *
                        (
                            ...
                            2 *
                                ...
                                sstatus->batk +
                            sstatus->rhw.atk +
                            (index >= 0 && sd->inventory_data[index]
                                 ? sd->inventory_data[index]->atk
                                 : 0)) *
                        (skill_lv + 5) / 5;
  ...
  default:
    ...
    if (sd)
      ...
      i = (is_attack_critical(wd, src, target, skill_id, skill_lv, false) ? 1
                                                                          : 0) |
          (!skill_id && sc && sc->data[SC_CHANGE] ? 4 : 0);

      wd->damage = battle_calc_base_damage(src, sstatus, &sstatus->rhw, sc,
                                           tstatus->size, i);
      if (is_attack_left_handed(src, skill_id))
        wd->damage2 = battle_calc_base_damage(src, sstatus, &sstatus->lhw, sc,
                                              tstatus->size, i);
    }
#else
    i = (is_attack_critical(wd, src, target, skill_id, skill_lv, false) ? 1
    ...
    if (sd) {
      ...
      if (sd->bonus.crit_atk_rate &&
          is_attack_critical(wd, src, target, skill_id, skill_lv,
                             false)) { // add +crit damage bonuses here in
                                       // pre-renewal mode [helvetica]
...
#ifndef RENEWAL
    if (tsd != nullptr && tsd->bonus.crit_def_rate != 0 && !skill_id &&
        is_attack_critical(wd, src, target, skill_id, skill_lv, false)) {
  ...
  if (d->damage) {
    ...
    if (d->damage < d->div_ &&
        (skill_id == SU_LUNATICCARROTBEAT ||
         skill_id == SU_LUNATICCARROTBEAT2 || skill_id == SU_CN_METEOR ||
         skill_id == SU_CN_METEOR2 ||
         (battle_config.skill_min_damage & d->flag)))
      d->damage = d->div_;
  } else if (d->div_ < 0) {
    d->div_ *= -1;
  }
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_multi_attack(struct Damage *wd, struct block_list *src,
                                     struct block_list *target, uint16 skill_id,
                                     uint16 skill_lv) {
  struct map_session_data *sd = BL_CAST(BL_PC, src);
  struct status_change *sc = status_get_sc(src);
  ...
  if (sd &&
      !skill_id) { // if no skill_id passed, check for double attack [helvetica]
    short i;
    if (((skill_lv = pc_checkskill(sd, TF_DOUBLE)) > 0 &&
         sd->weapontype1 == W_DAGGER) ||
        (sd->bonus.double_rate > 0 &&
         sd->weapontype1 != W_FIST) // Will fail bare-handed
        || (sc && sc->data[SC_KAGEMUSYA] &&
            sd->weapontype1 != W_FIST)) // Will fail bare-handed
    { // Success chance is not added, the higher one is used [Skotlex]
      int max_rate = 0;

      if (sc && sc->data[SC_KAGEMUSYA])
        max_rate = sc->data[SC_KAGEMUSYA]->val1 *
                   10; // Same rate as even levels of TF_DOUBLE
      else
        // #ifdef RENEWAL
    ...
    } else if (((sd->weapontype1 == W_REVOLVER &&
                 (skill_lv = pc_checkskill(sd, GS_CHAINACTION)) >
                     0) // Normal Chain Action effect
                || (sc && sc->count && sc->data[SC_E_CHAIN] &&
                    (skill_lv = sc->data[SC_E_CHAIN]->val1) >
                        0))                   // Chain Action of ETERNAL_CHAIN
               && rnd() % 100 < 5 * skill_lv) // Success rate
    {
      wd->div_ = skill_get_num(GS_CHAINACTION, skill_lv);
      wd->type = DMG_MULTI_HIT;

      sc_start(src, src, SC_QD_SHOT_READY, 100, target->id,
               skill_get_time(RL_QD_SHOT, 1));
    } else if (sc && sc->data[SC_FEARBREEZE] && sd->weapontype1 == W_BOW &&
               (i = sd->equip_index[EQI_AMMO]) >= 0 && sd->inventory_data[i] &&
 ...
 *	Refined and optimized by helvetica
 */
static int battle_calc_attack_skill_ratio(struct Damage *wd,
                                          struct block_list *src,
                                          struct block_list *target,
                                          uint16 skill_id, uint16 skill_lv) {
  struct map_session_data *sd = BL_CAST(BL_PC, src);
  ...
  if (sc && skill_id != PA_SACRIFICE) {
    ...
    if (!skill_id || skill_id == KN_AUTOCOUNTER) {
      if (sc->data[SC_CRUSHSTRIKE]) {
        if (sd) { // ATK [{Weapon Level * (Weapon Upgrade Level + 6) * 100} +
                  // (Weapon ATK) + (Weapon Weight)]%
          short index = sd->equip_index[EQI_HAND_R];

          if (index >= 0 && sd->inventory_data[index] &&
              sd->inventory_data[index]->type == IT_WEAPON)
            skillratio +=
                -100 + sd->inventory_data[index]->weight / 10 +
                sd->inventory_data[index]->atk +
                100 * sd->inventory_data[index]->wlv *
        ...
        if (sc->data[SC_GIANTGROWTH] &&
            (sd->class_ & MAPID_THIRDMASK) ==
                MAPID_RUNE_KNIGHT) { // Increase damage again if Crush Strike is
                                     // not active
          if (map_flag_vs(
                  src->m)) // Only half of the 2.5x increase on versus-type maps
            skillratio += 125;
          else
            skillratio += 250;
        }
  ...
  case MS_MAGNUM:
    if (wd->miscflag == 1)
      skillratio +=
          20 *
          skill_lv; // Inner 3x3 circle takes 100%+20%*level damage [Playtester]
    else
      skillratio +=
          10 *
          skill_lv; // Outer 5x5 circle takes 100%+10%*level damage [Playtester]
    break;
  case MC_MAMMONITE:
  ...
  case MC_CARTREVOLUTION:
    ...
    if (sd && sd->cart_weight)
      skillratio +=
          100 * sd->cart_weight / sd->cart_weight_max; // +1% every 1% weight
    else if (!sd)
  ...
  case MO_EXTREMITYFIST:
    ...
    skillratio =
        min(500000,
            skillratio); // We stop at roughly 50k SP for overflow protection
    break;
...
#ifdef RENEWAL
    skillratio +=
        450 + 50 * skill_lv + sstatus->str; // !TODO: How does STR play a role?
#else
...
#ifdef RENEWAL
    skillratio +=
        100 + 100 * skill_lv + sstatus->str; // !TODO: How does STR play a role?
    RE_LVL_DMOD(100);
#else
    skillratio += 100 + 100 * skill_lv;
#endif
  ...
  case LK_JOINTBEAT:
    ...
    if (wd->miscflag & BREAK_NECK ||
        (tsc && tsc->data[SC_JOINTBEAT] &&
         tsc->data[SC_JOINTBEAT]->val2 &
             BREAK_NECK)) // The 2x damage is only for the BREAK_NECK ailment.
      skillratio <<= 1;
    break;
#ifdef RENEWAL
  ...
  case SN_SHARPSHOOTING:
    if (src->type ==
        BL_MOB) { // TODO: Did these formulas change in the renewal balancing?
  ...
  case CR_ACIDDEMONSTRATION:
    skillratio += -100 + 200 * skill_lv + sstatus->int_ +
                  tstatus->vit; // !TODO: Confirm status bonus
  ...
  case ASC_BREAKER:
    ...
    skillratio += -100 + 150 * skill_lv + sstatus->str +
                  sstatus->int_; // !TODO: Confirm stat modifier
    RE_LVL_DMOD(100);
#else
  ...
  case WS_CARTTERMINATION:
    ...
    if (sd && sd->cart_weight)
      skillratio +=
          sd->cart_weight / i * 80000 / battle_config.max_cart_weight - 100;
    else if (!sd)
  ...
  case TK_JUMPKICK:
    ...
    if (sc && sc->data[SC_COMBO] && sc->data[SC_COMBO]->val1 == skill_id)
      skillratio +=
          -100 + 4 * status_get_lv(src); // Tumble formula [4%*baselevel]
    else if (wd->miscflag) {
      skillratio +=
          -100 + 4 * status_get_lv(src); // Running formula [4%*baselevel]
  ...
  case KN_CHARGEATK: { // +100% every 3 cells of distance but hard-limited to
                       // 500%
    int k = (wd->miscflag - 1) / 3;
    if (k < 0)
      k = 0;
  ...
  case RK_SONICWAVE:
    ...
    // skillratio += -100 + (skill_lv + 7) * 100; // ATK = {((Skill Level + 7) x
    // 100) x (1 + [(Caster's Base Level - 100) / 200])} %
    skillratio +=
        -100 + (skill_lv + 5) * 100; // ATK = {((Skill Level + 5) x 100) x (1 +
                                     // [(Caster's Base Level - 100) / 200])} %
  ...
  case RK_IGNITIONBREAK:
    ...
    // RE_LVL_DMOD(100);
    // 3x3 cell Damage = ATK [{(Skill Level x 300) x (1 + [(Caster's Base Level
    // - 100) / 100])}] % 7x7 cell Damage = ATK [{(Skill Level x 250) x (1 +
    // [(Caster's Base Level - 100) / 100])}] % 11x11 cell Damage = ATK [{(Skill
    // Level x 200) x (1 + [(Caster's Base Level - 100) / 100])}] %
  ...
  case NPC_IGNITIONBREAK:
    ...
    break;
  case RK_STORMBLAST:
    skillratio += -100 + (((sd) ? pc_checkskill(sd, RK_RUNEMASTERY) : 0) +
                          sstatus->str / 6) *
                             100; // ATK = [{Rune Mastery Skill Level +
                                  // (Caster's STR / 6)} x 100] %
    RE_LVL_DMOD(100);
    break;
  case RK_PHANTOMTHRUST: // ATK = [{(Skill Level x 50) + (Spear Master Level x
                         // 10)} x Caster's Base Level / 150] %
    skillratio += -100 + 50 * skill_lv +
                  10 * (sd ? pc_checkskill(sd, KN_SPEARMASTERY) : 5);
  ...
  case GC_COUNTERSLASH:
    // ATK [{(Skill Level x 100) + 300} x Caster's Base Level / 120]% + ATK
    // [(AGI x 2) + (Caster's Job Level x 4)]%
    skillratio += 200 + (100 * skill_lv);
    // ATK [{(Skill Level x 150) + 300} x Caster's Base Level / 120]% + ATK
    // [(AGI x 2) + (Caster's Job Level x 4)]%
  ...
  case RA_ARROWSTORM:
    ...
    if (sc && sc->data[SC_CAMOUFLAGE] && sc->data[SC_CAMOUFLAGE]->val3 >= 10 &&
        !sc->data[SC_UNLIMIT])
  ...
  case NC_BOOSTKNUCKLE:
    ...
    RE_LVL_DMOD(120);
    // skillratio += 200 * skill_lv + sstatus->dex / 6; // !TODO: What's the DEX
    // bonus? RE_LVL_DMOD(100);
    break;
  case NC_PILEBUNKER:
  ...
  case NC_VULCANARM:
    skillratio += -100 + 140 * skill_lv +
                  sstatus->dex / 6; // !TODO: What's the DEX bonus?
    RE_LVL_DMOD(100);
    break;
  ...
  case NC_AXEBOOMERANG:
    ...
    if (sd) {
      ...
      if (index >= 0 && sd->inventory_data[index] &&
          sd->inventory_data[index]->type == IT_WEAPON)
        skillratio += sd->inventory_data[index]->weight /
                      10; // Weight is divided by 10 since 10 weight in coding
                          // make 1 whole actual weight. [Rytech]
    }
    RE_LVL_DMOD(100);
    break;
  case NC_POWERSWING: // According to current sources, only the str + dex gets
                      // modified by level [Akinari]
    skillratio += -100 + status_get_str(src) + status_get_dex(src);
    // skillratio += -100 + ((sstatus->str + sstatus->dex)/ 2) + 300 + 100 *
    // skill_lv;
    RE_LVL_DMOD(100);
    break;
  ...
  case NC_AXETORNADO:
    skillratio += -100 + 200 + 180 * skill_lv +
                  sstatus->vit / 6; // !TODO: What's the VIT bonus?
    RE_LVL_DMOD(100);
    break;
  case SC_FATALMENACE:
    skillratio +=
        120 * skill_lv + sstatus->agi / 6; // !TODO: What's the AGI bonus?
    RE_LVL_DMOD(100);
    break;
  case SC_TRIANGLESHOT:
    ...
    break;
  case SC_FEINTBOMB:
    skillratio += -100 + (skill_lv + 1) * sstatus->dex / 2 *
                             ((sd) ? sd->status.job_level / 10 : 1);
    RE_LVL_DMOD(120);
    break;
  case LG_CANNONSPEAR:
  ...
  case LG_BANISHINGPOINT:
    skillratio +=
        -100 + (80 * skill_lv) + ((sd) ? pc_checkskill(sd, SM_BASH) * 30 : 0);
    RE_LVL_DMOD(100);
    break;
  case LG_SHIELDPRESS:
  ...
  case LG_RAGEBURST:
    if (sd && sd->spiritball_old)
      skillratio += -100 + 200 * sd->spiritball_old +
                    (status_get_max_hp(src) - status_get_hp(src)) / 100;
    else
      skillratio += 2900 + (status_get_max_hp(src) - status_get_hp(src));
    RE_LVL_DMOD(100);
    break;
  case LG_SHIELDSPELL:
    if (sd && skill_lv == 1) { // [(Casters Base Level x 4) + (Shield DEF x 10)
                               // + (Casters VIT x 2)] %
      short index = sd->equip_index[EQI_HAND_L];
  ...
  case LG_MOONSLASHER:
    skillratio += -100 + 120 * skill_lv +
                  ((sd) ? pc_checkskill(sd, LG_OVERBRAND) * 80 : 0);
    RE_LVL_DMOD(100);
    break;
  case LG_OVERBRAND:
    skillratio += -100 + 400 * skill_lv +
                  ((sd) ? pc_checkskill(sd, CR_SPEARQUICKEN) * 50 : 0);
    RE_LVL_DMOD(100);
  ...
  case LG_OVERBRAND_BRANDISH:
    skillratio +=
        -100 + 300 * skill_lv + status_get_str(src) + status_get_dex(src);
    RE_LVL_DMOD(100);
    break;
  case LG_OVERBRAND_PLUSATK: // Only Piercing and Swing damage get RE_LVL_DMOD
                             // bonus damage
  ...
  case LG_EARTHDRIVE:
    skillratio +=
        -100 + 380 * skill_lv +
        ((sstatus->str + sstatus->vit) / 6); // !TODO: What's the STR/VIT bonus?
    RE_LVL_DMOD(100);
    break;
  case LG_HESPERUSLIT:
  ...
  case SR_TIGERCANNON: {
    ...
    if (wd->miscflag & 8)
      // Base_Damage = [((Caster consumed HP + SP) / 2) x Caster Base Level /
      // 100] %
      skillratio += -100 + (hp + sp) / 2;
    else
      // Base_Damage = [((Caster consumed HP + SP) / 4) x Caster Base Level /
      // 100] %
  ...
  case SR_SKYNETBLOW:
    ...
    skillratio +=
        -100 + 200 * skill_lv + sstatus->agi / 6; // !TODO: Confirm AGI bonus
    RE_LVL_DMOD(100);
    break;
  ...
  case SR_KNUCKLEARROW:
    if (wd->miscflag & 4) { // ATK [(Skill Level x 150) + (1000 x Target current
                            // weight / Maximum weight) + (Target Base Level x
                            // 5) x (Caster Base Level / 150)] %
    ...
    // skillratio += skillratio * 30 / 100;
    break;
  case SR_WINDMILL: // ATK [(Caster Base Level + Caster DEX) x Caster Base Level
                    // / 100] %
  ...
  case WM_SEVERE_RAINSTORM_MELEE:
    ...
    // skillratio += -100 + 100 * skill_lv + (sstatus->dex / 300 + sstatus->agi
    // / 200);
  ...
  case WM_GREAT_ECHO:
    ...
    RE_LVL_DMOD(100);
    break;
  case GN_CART_TORNADO: { // ATK [( Skill Level x 200 ) + ( Cart Weight / ( 150
                          // - Caster Base STR ))] + ( Cart Remodeling Skill
                          // Level x 50 )] %
                          // skillratio += -100 + 200 * skill_lv;
    skillratio += -100 + 50 * skill_lv;
    if (sd && sd->cart_weight)
  ...
  case GN_CARTCANNON:
    // ATK [{( Cart Remodeling Skill Level x 50 ) x ( INT / 40 )} + ( Cart
    // Cannon Skill Level x 60 )] %
    skillratio += -100 + 60 * skill_lv +
                  ((sd) ? pc_checkskill(sd, GN_REMODELING_CART) : 1) * 50 *
                      status_get_int(src) / 40;
    // skillratio += -100 + (250 + 20 * pc_checkskill(sd, GN_REMODELING_CART)) *
    // skill_lv + 2 * sstatus->int_ / (6 - pc_checkskill(sd,
    // GN_REMODELING_CART)); RE_LVL_DMOD(100);
    break;
  ...
  case GN_SLINGITEM_RANGEMELEEATK:
    if (sd) {
      ...
      case ITEMID_BLACK_LUMP:
        skillratio += -100 + (status_get_str(src) + status_get_agi(src) +
                              status_get_dex(src)) /
                                 3;
        break;
  ...
  case KO_HUUMARANKA:
    skillratio += -100 + 150 * skill_lv + sstatus->str +
                  (sd ? pc_checkskill(sd, NJ_HUUMA) * 100 : 0);
    RE_LVL_DMOD(100);
    if (sc && sc->data[SC_KAGEMUSYA])
      skillratio += skillratio * sc->data[SC_KAGEMUSYA]->val2 / 100;
    break;
  case KO_SETSUDAN:
    ...
    if (tsc) {
      ...
      if ((sce = tsc->data[SC_SPIRIT]) || (sce = tsc->data[SC_SOULGOLEM]) ||
          (sce = tsc->data[SC_SOULSHADOW]) ||
          (sce = tsc->data[SC_SOULFALCON]) ||
          (sce = tsc->data[SC_SOULFAIRY])) // Bonus damage added when target is
                                           // soul linked.
        skillratio += 200 * sce->val1;
    }
    break;
  case KO_BAKURETSU:
    skillratio += -100 + (sd ? pc_checkskill(sd, NJ_TOBIDOUGU) : 1) *
                             (50 + sstatus->dex / 4) * skill_lv * 4 / 10;
    RE_LVL_DMOD(120);
  ...
  case MH_NEEDLE_OF_PARALYZE:
    skillratio += -100 + 450 * skill_lv * status_get_lv(src) / 100 +
                  sstatus->dex / 6; // !TODO: Confirm Base Level and DEX bonus
    break;
  case MH_STAHL_HORN:
    skillratio += -100 + 1000 + 300 * skill_lv * status_get_lv(src) / 150 +
                  sstatus->vit / 6; // !TODO: Confirm VIT bonus
    break;
  case MH_LAVA_SLIDE:
    skillratio += -100 + 50 * skill_lv;
    break;
  ...
  case MH_SILVERVEIN_RUSH:
    skillratio += -100 + 250 * skill_lv * status_get_lv(src) / 100 +
                  sstatus->str / 6; // !TODO: Confirm STR bonus
    break;
  case MH_MIDNIGHT_FRENZY:
    skillratio += -100 + 450 * skill_lv * status_get_lv(src) / 150 +
                  sstatus->str / 6; // !TODO: Confirm STR bonus
    break;
  case MH_MAGMA_FLOW:
    skillratio += -100 + (100 * skill_lv + 3 * status_get_lv(src)) *
                             status_get_lv(src) / 120;
    break;
  case RL_MASS_SPIRAL:
    skillratio += -100 + 200 * skill_lv;
    break;
...
/*==================================================================================================
 * Constant skill damage additions are added before SC modifiers and after skill
 ...
 *	Refined and optimized by helvetica
 */
static int64 battle_calc_skill_constant_addition(struct Damage *wd,
                                                 struct block_list *src,
                                                 struct block_list *target,
                                                 uint16 skill_id,
                                                 uint16 skill_lv) {
  ...
  case LG_SHIELDPRESS:
    if (sd) {
      ...
      if (index >= 0 && sd->inventory_data[index] &&
          sd->inventory_data[index]->type == IT_ARMOR)
        damagevalue =
            sstatus->vit * sd->inventory.u.items_inventory[index].refine;
      atk = damagevalue;
    }
    break;
  case SR_FALLENEMPIRE:
    // [(Target Size value + Skill Level - 1) x Caster STR] + [(Target current
    // weight x Caster DEX / 120)]
 ...
 *	Refined and optimized by helvetica
 */
static void battle_attack_sc_bonus(struct Damage *wd, struct block_list *src,
                                   struct block_list *target, uint16 skill_id,
                                   uint16 skill_lv) {
  struct map_session_data *sd = BL_CAST(BL_PC, src);
  struct status_change *sc = status_get_sc(src);
  ...
  if (sc) {
    ...
    if (sc->data[SC_WATK_ELEMENT] && skill_id != ASC_METEORASSAULT)
      ATK_ADDRATE(wd->weaponAtk, wd->weaponAtk2,
                  sc->data[SC_WATK_ELEMENT]->val2);
    ...
    if (sc->data[SC_MAGICALBULLET]) {
      ...
      if (sstatus->matk_min > tmdef && sstatus->matk_max > sstatus->matk_min) {
        ATK_ADD(wd->weaponAtk, wd->weaponAtk2,
                i64max((sstatus->matk_min +
                        rnd() % (sstatus->matk_max - sstatus->matk_min)) -
                           tmdef,
                       0));
      } else {
        ATK_ADD(wd->weaponAtk, wd->weaponAtk2,
                i64max(sstatus->matk_min - tmdef, 0));
      }
    }
    ...
    if (sc->data[SC_SPIRIT]) {
      if (skill_id == AS_SONICBLOW && sc->data[SC_SPIRIT]->val2 == SL_ASSASIN) {
        ATK_ADDRATE(wd->damage, wd->damage2,
                    map_flag_gvg2(src->m)
                        ? 25
                        : 100); //+25% dmg on woe/+100% dmg on nonwoe
        RE_ALLATK_ADDRATE(wd, map_flag_gvg2(src->m)
                                  ? 25
                                  : 100); //+25% dmg on woe/+100% dmg on nonwoe
      } else if (skill_id == CR_SHIELDBOOMERANG &&
                 sc->data[SC_SPIRIT]->val2 == SL_CRUSADER) {
    ...
    if (sc->data[SC_EDP]) {
      ...
      default: // fall through to apply EDP bonuses
        ...
        ATK_RATE(wd->weaponAtk, wd->weaponAtk2,
                 100 + (sc->data[SC_EDP]->val1 * 80));
        ATK_RATE(wd->equipAtk, wd->equipAtk2,
                 100 + (sc->data[SC_EDP]->val1 * 60));
        break;
#else
      default:
    ...
    if (sc->data[SC_DANCEWITHWUG]) {
      if (skill_get_inf2(skill_id, INF2_INCREASEDANCEWITHWUGDAMAGE)) {
        ATK_ADDRATE(wd->damage, wd->damage2,
                    sc->data[SC_DANCEWITHWUG]->val1 * 10 *
                        battle_calc_chorusbonus(sd));
        RE_ALLATK_ADDRATE(wd, sc->data[SC_DANCEWITHWUG]->val1 * 10 *
                                  battle_calc_chorusbonus(sd));
      }
      ATK_ADDRATE(wd->damage, wd->damage2,
                  sc->data[SC_DANCEWITHWUG]->val1 * 2 *
                      battle_calc_chorusbonus(sd));
#ifdef RENEWAL
      ATK_ADDRATE(wd->equipAtk, wd->equipAtk2,
                  sc->data[SC_DANCEWITHWUG]->val1 * 2 *
                      battle_calc_chorusbonus(sd));
#endif
    }
    ...
    if (sd && wd->flag & BF_WEAPON && sc->data[SC_GVG_GIANT] &&
        sc->data[SC_GVG_GIANT]->val3) {
    ...
    if (sc->data[SC_MIRACLE])
      anger_id = 2; // Always treat all monsters as star flagged monster when in
                    // miracle state
  }

  if ((wd->flag & (BF_LONG | BF_MAGIC)) == BF_LONG) {
    if (sd && pc_checkskill(sd, SU_POWEROFLIFE) > 0 &&
        pc_checkskill_summoner(sd, SUMMONER_POWER_LIFE) >= 20) {
  ...
  if (sd != nullptr && !anger_id)
    ARR_FIND(0, MAX_PC_FEELHATE, anger_id,
             status_get_class(target) == sd->hate_mob[anger_id]);

  uint16 anger_level;
  if (sd != nullptr && anger_id < MAX_PC_FEELHATE &&
      (anger_level = pc_checkskill(sd, sg_info[anger_id].anger_id))) {
    int skillratio = sd->status.base_level + sstatus->dex + sstatus->luk;

    if (anger_id == 2)
      skillratio +=
          sstatus
              ->str; // SG_STAR_ANGER additionally has STR added in its formula.
    if (anger_level < 4)
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_defense_reduction(struct Damage *wd,
                                          struct block_list *src,
                                          struct block_list *target,
                                          uint16 skill_id, uint16 skill_lv) {
  struct map_session_data *sd = BL_CAST(BL_PC, src);
  ...
  // Defense reduction
  short vit_def;
  defType def1 = status_get_def(
      target); // Don't use tstatus->def1 due to skill timer reductions.
  short def2 = tstatus->def2;

  if (sd) {
    int i = sd->indexed_bonus.ignore_def_by_race[tstatus->race] +
            sd->indexed_bonus.ignore_def_by_race[RC_ALL];
    i += sd->indexed_bonus.ignore_def_by_class[tstatus->class_] +
         sd->indexed_bonus.ignore_def_by_class[CLASS_ALL];
    if (i) {
  ...
  if (battle_config.vit_penalty_type &&
    ...
    target_count = min(unit_counttargeted(target),
                       (100 / battle_config.vit_penalty_num) +
                           (battle_config.vit_penalty_count - 1));
    if (target_count >= battle_config.vit_penalty_count) {
      if (battle_config.vit_penalty_type == 1) {
        if (!tsc || !tsc->data[SC_STEELBODY])
          def1 =
              (def1 *
               (100 - (target_count - (battle_config.vit_penalty_count - 1)) *
        ...
        if (!tsc || !tsc->data[SC_STEELBODY])
          def1 -= (target_count - (battle_config.vit_penalty_count - 1)) *
                  battle_config.vit_penalty_num;
        def2 -= (target_count - (battle_config.vit_penalty_count - 1)) *
                battle_config.vit_penalty_num;
      }
    }
  ...
  // Vitality reduction from rodatazone:
  ...
  if (tsd) { // Sd vit-eq
    ...
    if (src->type == BL_MOB &&
        (battle_check_undead(sstatus->race, sstatus->def_ele) ||
         sstatus->race ==
             RC_DEMON) && // This bonus already doesn't work vs players
        (skill = pc_checkskill(tsd, AL_DP)) > 0)
      vit_def += skill * (int)(3 + (tsd->status.base_level + 1) *
                                       0.04); // submitted by orn
    if (src->type == BL_MOB &&
        (skill = pc_checkskill(tsd, RA_RANGERMAIN)) > 0 &&
        (sstatus->race == RC_BRUTE || sstatus->race == RC_PLAYER_DORAM ||
         sstatus->race == RC_FISH || sstatus->race == RC_PLANT))
      vit_def += skill * 5;
    if (src->type == BL_MOB &&
        (skill = pc_checkskill(tsd, NC_RESEARCHFE)) > 0 &&
  ...
  ATK_ADD2(wd->damage, wd->damage2,
           is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R)
               ? (def1 * battle_calc_attack_skill_ratio(wd, src, target,
                                                        skill_id, skill_lv)) /
                     200
               : 0,
  ...
  ATK_RATE2(
      ...
      attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_R)
          ? 100
          : (is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R)
                 ? (int64)is_attack_piercing(wd, src, target, skill_id,
                                             skill_lv, EQI_HAND_R) *
                       (def1 + vit_def)
                 : (100 - def1)),
      attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_L)
  ...
  ATK_ADD2(
      wd->damage, wd->damage2,
      attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_R) ||
              is_attack_piercing(wd, src, target, skill_id, skill_lv,
                                 EQI_HAND_R)
          ? 0
          : -vit_def,
      attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_L) ||
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_post_defense(struct Damage *wd,
                                            struct block_list *src,
                                            struct block_list *target,
                                            uint16 skill_id, uint16 skill_lv) {
  struct map_session_data *sd = BL_CAST(BL_PC, src);
...
#ifdef RENEWAL
      ATK_ADD(wd->damage, wd->damage2,
              (3 + sc->data[SC_AURABLADE]->val1) *
                  status_get_lv(src)); // !TODO: Confirm formula
#else
  ...
  if (sd && battle_skill_stacks_masteries_vvs(skill_id) &&
      ...
      skill_id != MO_EXTREMITYFIST) { // Counts refine bonus multiple times
    if (skill_id == MO_FINGEROFFENSIVE) {
      ATK_ADD2(wd->damage, wd->damage2, wd->div_ * sstatus->rhw.atk2,
               wd->div_ * sstatus->lhw.atk2);
    } else {
...
/*=================================================================================
 * "Plant"-type (mobs that only take 1 damage from all sources) damage
 *calculation
 *---------------------------------------------------------------------------------
 * Credits:
 *	Original coder Skotlex
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_plant(struct Damage *wd, struct block_list *src,
                                     struct block_list *target, uint16 skill_id,
                                     uint16 skill_lv) {
  struct status_data *tstatus = status_get_status_data(target);
  bool attack_hits =
      is_attack_hitting(wd, src, target, skill_id, skill_lv, false);
  ...
  if (attack_hits || wd->damage > 0)
    wd->damage = 1; // In some cases, right hand no need to have a weapon to
                    // deal a damage
  if (is_attack_left_handed(src, skill_id) &&
      (attack_hits || wd->damage2 > 0)) {
    struct map_session_data *sd = BL_CAST(BL_PC, src);
    ...
    if (sd && sd->status.weapon == W_KATAR)
      ...
      wd->damage2 =
          1; // Deal 1 HP damage as long as there is a weapon in the left hand
  }
  ...
  if (attack_hits && target->type == BL_MOB) {
    ...
    if (sc && !battle_status_block_damage(
                  src, target, sc, wd, damage_dummy, skill_id,
                  skill_lv)) { // Statuses that reduce damage to 0.
  ...
  if (attack_hits && status_get_class(target) == MOBID_EMPERIUM) {
    if (target &&
        !battle_can_hit_gvg_target(src, target, skill_id,
                                   (skill_id) ? BF_SKILL : 0) &&
    ...
    const int right_element = battle_get_weapon_element(
        wd, src, target, skill_id, skill_lv, EQI_HAND_R, false);
    const int left_element = battle_get_weapon_element(
        wd, src, target, skill_id, skill_lv, EQI_HAND_L, false);

    if (wd->damage > 0) {
      wd->damage = battle_attr_fix(src, target, wd->damage, right_element,
                                   tstatus->def_ele, tstatus->ele_lv);
      wd->damage =
          battle_calc_gvg_damage(src, target, wd->damage, skill_id, wd->flag);
    } else if (wd->damage2 > 0) {
      wd->damage2 = battle_attr_fix(src, target, wd->damage2, left_element,
                                    tstatus->def_ele, tstatus->ele_lv);
      wd->damage2 =
          battle_calc_gvg_damage(src, target, wd->damage2, skill_id, wd->flag);
    }
    return;
  }

  // For plants we don't continue with the weapon attack code, so we have to
  // apply DAMAGE_DIV_FIX here
  battle_apply_div_fix(wd, skill_id);

  // If there is left hand damage, total damage can never exceed 2, even on
  // multiple hits
...
/*========================================================================================
 * Perform left/right hand weapon damage calculation based on previously
 *calculated damage
 *----------------------------------------------------------------------------------------
 * Credits:
 *	Original coder Skotlex
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_left_right_hands(struct Damage *wd,
                                                struct block_list *src,
                                                struct block_list *target,
                                                uint16 skill_id,
                                                uint16 skill_lv) {
  ...
  if (sd) {
    ...
    if (!is_attack_right_handed(src, skill_id) &&
        is_attack_left_handed(src, skill_id)) {
      wd->damage = wd->damage2;
      wd->damage2 = 0;
    } else if (sd->status.weapon == W_KATAR &&
               !skill_id) { // Katars (offhand damage only applies to normal
                            // attacks, tested on Aegis 10.2)
      skill = pc_checkskill(sd, TF_DOUBLE);
      wd->damage2 = (int64)wd->damage * (1 + (skill * 2)) / 100;
    } else if (is_attack_right_handed(src, skill_id) &&
               is_attack_left_handed(src, skill_id)) { // Dual-wield
      if (wd->damage) {
  ...
  if (!is_attack_right_handed(src, skill_id) &&
      !is_attack_left_handed(src, skill_id) && wd->damage)
    wd->damage = 0;
...
struct block_list *battle_check_devotion(struct block_list *bl) {
  ...
  if (battle_config.devotion_rdamage &&
      battle_config.devotion_rdamage > rnd() % 100) {
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_gvg_bg(struct Damage *wd, struct block_list *src,
                                      struct block_list *target,
                                      uint16 skill_id, uint16 skill_lv) {
  if (wd->damage + wd->damage2) { // There is a total damage value
    if (src != target &&          // Don't reflect your own damage (Grand Cross)
        (!skill_id || skill_id ||
         (src->type == BL_SKILL &&
          (skill_id == SG_SUN_WARM || skill_id == SG_MOON_WARM ||
      ...
      rdamage = battle_calc_return_damage(target, src, &damage, wd->flag,
                                          skill_id, false);
      if (rdamage > 0) { // Item reflect gets calculated before any mapflag
                         // reducing is applicated
        struct block_list *d_bl = battle_check_devotion(src);

        rdelay =
            clif_damage(src, (!d_bl) ? src : d_bl, tick, wd->amotion,
                        sstatus->dmotion, rdamage, 1, DMG_ENDURE, 0, false);
        if (tsd)
          battle_drain(tsd, src, rdamage, rdamage, sstatus->race,
                       sstatus->class_);
        // Use Reflect Shield to signal this kind of skill trigger [Skotlex]
        battle_delay_damage(tick, wd->amotion, target, (!d_bl) ? src : d_bl, 0,
                            CR_REFLECTSHIELD, 0, rdamage, ATK_DEF, rdelay, true,
                            false);
        skill_additional_effect(target, (!d_bl) ? src : d_bl, CR_REFLECTSHIELD,
                                1, BF_WEAPON | BF_SHORT | BF_NORMAL, ATK_DEF,
                                tick);
      }
    }
    ...
    if (!wd->damage2) {
      wd->damage =
          battle_calc_damage(src, target, wd, wd->damage, skill_id, skill_lv);
      if (mapdata_flag_gvg2(mapdata))
        wd->damage =
            battle_calc_gvg_damage(src, target, wd->damage, skill_id, wd->flag);
      else if (mapdata->flag[MF_BATTLEGROUND])
        wd->damage =
            battle_calc_bg_damage(src, target, wd->damage, skill_id, wd->flag);
      if (battle_config.atk_adjustment_map)
        wd->damage = battle_calc_damage_adjustment(
            src, wd->damage, wd->flag); // Global damage adjustment [Cydh]
    } else if (!wd->damage) {
      wd->damage2 =
          battle_calc_damage(src, target, wd, wd->damage2, skill_id, skill_lv);
      if (mapdata_flag_gvg2(mapdata))
        wd->damage2 = battle_calc_gvg_damage(src, target, wd->damage2, skill_id,
                                             wd->flag);
      else if (mapdata->flag[MF_BATTLEGROUND])
        wd->damage2 =
            battle_calc_bg_damage(src, target, wd->damage2, skill_id, wd->flag);
      if (battle_config.atk_adjustment_map)
        wd->damage = battle_calc_damage_adjustment(
            src, wd->damage, wd->flag); // Global damage adjustment [Cydh]
    } else {
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_weapon_final_atk_modifiers(struct Damage *wd,
                                                   struct block_list *src,
                                                   struct block_list *target,
                                                   uint16 skill_id,
                                                   uint16 skill_lv) {
  ...
  if (wd->damage && tsc && tsc->data[SC_REJECTSWORD] &&
    ...
    status_fix_damage(target, src, wd->damage,
                      clif_damage(target, src, gettick(), 0, 0, wd->damage, 0,
                                  DMG_NORMAL, 0, false),
                      ST_REJECTSWORD);
    clif_skill_nodamage(target, target, ST_REJECTSWORD,
                        tsc->data[SC_REJECTSWORD]->val1, 1);
    if (--(tsc->data[SC_REJECTSWORD]->val3) <= 0)
      status_change_end(target, SC_REJECTSWORD, INVALID_TIMER);
  }

  if (tsc && tsc->data[SC_CRESCENTELBOW] && wd->flag & BF_SHORT &&
      rnd() % 100 < tsc->data[SC_CRESCENTELBOW]->val2) {
    // ATK [{(Target HP / 100) x Skill Level} x Caster Base Level / 125] % +
    // [Received damage x {1 + (Skill Level x 0.2)}]
    int64 rdamage = 0;
    int ratio = (int64)(status_get_hp(src) / 100) *
                tsc->data[SC_CRESCENTELBOW]->val1 * status_get_lv(target) / 125;
    if (ratio > 5000)
      ratio = 5000; // Maximum of 5000% ATK
    rdamage = battle_calc_base_damage(target, tstatus, &tstatus->rhw, tsc,
                                      sstatus->size, 0);
    rdamage =
        (int64)rdamage * ratio / 100 +
        wd->damage * (10 + tsc->data[SC_CRESCENTELBOW]->val1 * 20 / 10) / 10;
    skill_blown(target, src,
                skill_get_blewcount(SR_CRESCENTELBOW_AUTOSPELL,
                                    tsc->data[SC_CRESCENTELBOW]->val1),
                unit_getdir(src), BLOWN_NONE);
    clif_skill_damage(target, src, gettick(), status_get_amotion(src), 0,
                      rdamage, 1, SR_CRESCENTELBOW_AUTOSPELL,
                      tsc->data[SC_CRESCENTELBOW]->val1,
                      DMG_SINGLE); // This is how official does
    clif_damage(src, target, gettick(), status_get_amotion(src) + 1000, 0,
                rdamage / 10, 1, DMG_NORMAL, 0, false);
  ...
  if (sc) {
    ...
    if (!skill_id && wd->dmg_lv > ATK_BLOCK) {
      if (sc->data[SC_ENCHANTBLADE]) {
        //[((Skill Lv x 20) + 100) x (casterBaseLevel / 150)] + casterInt + MATK
        //- MDEF - MDEF2
  ...
  if (skill_id == ASC_BREAKER) { // Breaker's int-based damage (a misc attack?)
    struct Damage md =
        battle_calc_misc_attack(src, target, skill_id, skill_lv, wd->miscflag);
 ...
 *	Refined and optimized by helvetica
 */
static struct Damage initialize_weapon_data(struct block_list *src,
                                            struct block_list *target,
                                            uint16 skill_id, uint16 skill_lv,
                                            int wflag) {
  struct status_data *sstatus = status_get_status_data(src);
  ...
  // counter attack DOES obey ASPD delay on official, uncomment if you want the
  // old (bad) behavior [helvetica]
  ...
  wd.flag |= (skill_id || wd.miscflag)
                 ? BF_SKILL
                 : BF_NORMAL; // Baphomet card's splash damage is counted as a
                              // skill. [Inkfish]
  wd.isspdamage = false;
  wd.damage = wd.damage2 =
#ifdef RENEWAL
      wd.statusAtk = wd.statusAtk2 = wd.equipAtk = wd.equipAtk2 = wd.weaponAtk =
          wd.weaponAtk2 = wd.masteryAtk = wd.masteryAtk2 =
#endif
              0;
  ...
  if (skill_id) {
    ...
    case LK_SPIRALPIERCE:
      if (!sd)
        wd.flag =
            (wd.flag & ~(BF_RANGEMASK | BF_WEAPONMASK)) | BF_LONG | BF_MISC;
      break;
    case RK_WINDCUTTER:
      if (sd &&
          (sd->status.weapon == W_1HSPEAR || sd->status.weapon == W_2HSPEAR))
        wd.flag |= BF_LONG;
      // The number of hits is set to 3 by default for use in Inspiration
      // status. When in Banding, the number of hits is equal to the number of
      // Royal Guards in Banding.
    case LG_HESPERUSLIT:
 ...
 */
void battle_do_reflect(int attack_type, struct Damage *wd,
  ...
  if ((wd->damage + wd->damage2) && src && target && src != target &&
    ...
    if ((ud && !ud->immune_attack) ||
        !status_bl_has_mode(target, MD_SKILLIMMUNE))
      rdamage = battle_calc_return_damage(target, src, &damage, wd->flag,
                                          skill_id, true);
    if (rdamage > 0) {
      struct block_list *d_bl = battle_check_devotion(src);
      ...
      if (tsc->data[SC_MAXPAIN]) {
        ...
        skill_castend_damage_id(target, src, NPC_MAXPAIN_ATK,
                                tsc->data[SC_MAXPAIN]->val1, tick, wd->flag);
        tsc->data[SC_MAXPAIN]->val2 = 0;
      } else if (attack_type == BF_WEAPON &&
                 tsc->data[SC_REFLECTDAMAGE]) // Don't reflect your own damage
                                              // (Grand Cross)
        map_foreachinshootrange(battle_damage_area, target,
                                skill_get_splash(LG_REFLECTDAMAGE, 1), BL_CHAR,
                                tick, target, wd->amotion, sstatus->dmotion,
                                rdamage, wd->flag);
        ...
        if (tsd)
          battle_drain(tsd, src, rdamage, rdamage, sstatus->race,
                       sstatus->class_);
        // It appears that official servers give skill reflect damage a longer
        // delay
        battle_delay_damage(tick, wd->amotion, target, (!d_bl) ? src : d_bl, 0,
                            CR_REFLECTSHIELD, 0, rdamage, ATK_DEF, rdelay, true,
                            false);
 ...
 *	Refined and optimized by helvetica
 */
static struct Damage battle_calc_weapon_attack(struct block_list *src,
                                               struct block_list *target,
                                               uint16 skill_id, uint16 skill_lv,
                                               int wflag) {
  ...
  right_element = battle_get_weapon_element(&wd, src, target, skill_id,
                                            skill_lv, EQI_HAND_R, false);
  left_element = battle_get_weapon_element(&wd, src, target, skill_id, skill_lv,
                                           EQI_HAND_L, false);
  ...
  if ((!skill_id || skill_id == PA_SACRIFICE) && tstatus->flee2 &&
      rnd() % 1000 < tstatus->flee2) {
  ...
  // on official check for multi hit first so we can override crit on double
  // attack [helvetica]
  ...
  if (!is_attack_hitting(&wd, src, target, skill_id, skill_lv, true))
    wd.dmg_lv = ATK_FLEE;
  else if (!(infdef = is_infinite_defense(
                 target, wd.flag))) { // no need for math against plants
    int64 ratio = 0;
    int i = 0;

    battle_calc_skill_base_damage(&wd, src, target, skill_id,
                                  skill_lv); // base skill damage
    ratio = battle_calc_attack_skill_ratio(&wd, src, target, skill_id,
                                           skill_lv); // skill level ratios

    ATK_RATE(wd.damage, wd.damage2, ratio);
    RE_ALLATK_RATE(&wd, ratio);

    int64 bonus_damage = battle_calc_skill_constant_addition(
        &wd, src, target, skill_id, skill_lv); // other skill bonuses

    ATK_ADD(wd.damage, wd.damage2, bonus_damage);

#ifdef RENEWAL
    if (skill_id ==
        HW_MAGICCRASHER) { // Add weapon attack for MATK onto Magic Crasher
      struct status_data *sstatus = status_get_status_data(src);
    ...
    // Card Fix for attacker (sd), 2 is added to the "left" flag meaning
    // "attacker cards only"
    if (sd) {
      wd.weaponAtk +=
          battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                              left_element, wd.weaponAtk, 2, wd.flag);
      wd.equipAtk +=
          battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                              left_element, wd.equipAtk, 2, wd.flag);
      if (is_attack_left_handed(src, skill_id)) {
        wd.weaponAtk2 +=
            battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                                left_element, wd.weaponAtk2, 3, wd.flag);
        wd.equipAtk2 +=
            battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                                left_element, wd.equipAtk2, 3, wd.flag);
      }
    }

    // Card Fix for target (tsd), 2 is not added to the "left" flag meaning
    // "target cards only"
    if (tsd && skill_id != NJ_ISSEN &&
        skill_id !=
            GN_FIRE_EXPANSION_ACID) { // These skills will do a card fix later
      std::bitset<NK_MAX> ignoreele_nk = nk;

      ignoreele_nk.set(NK_IGNOREELEMENT);
      wd.statusAtk += battle_calc_cardfix(BF_WEAPON, src, target, ignoreele_nk,
                                          right_element, left_element,
                                          wd.statusAtk, 0, wd.flag);
      wd.weaponAtk +=
          battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                              left_element, wd.weaponAtk, 0, wd.flag);
      wd.equipAtk +=
          battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                              left_element, wd.equipAtk, 0, wd.flag);
      wd.masteryAtk += battle_calc_cardfix(BF_WEAPON, src, target, ignoreele_nk,
                                           right_element, left_element,
                                           wd.masteryAtk, 0, wd.flag);
      if (is_attack_left_handed(src, skill_id)) {
        wd.statusAtk2 += battle_calc_cardfix(
            BF_WEAPON, src, target, ignoreele_nk, right_element, left_element,
            wd.statusAtk2, 1, wd.flag);
        wd.weaponAtk2 +=
            battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                                left_element, wd.weaponAtk2, 1, wd.flag);
        wd.equipAtk2 +=
            battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                                left_element, wd.equipAtk2, 1, wd.flag);
        wd.masteryAtk2 += battle_calc_cardfix(
            BF_WEAPON, src, target, ignoreele_nk, right_element, left_element,
            wd.masteryAtk2, 1, wd.flag);
      }
    }
    ...
    if (sd) { // monsters, homuns and pets have their damage computed directly
      wd.damage = wd.statusAtk + wd.weaponAtk + wd.equipAtk + wd.masteryAtk +
                  bonus_damage;
      if (is_attack_left_handed(src, skill_id))
        wd.damage2 = wd.statusAtk2 + wd.weaponAtk2 + wd.equipAtk2 +
                     wd.masteryAtk2 + bonus_damage;
      if (wd.flag & BF_SHORT)
        ATK_ADDRATE(wd.damage, wd.damage2, sd->bonus.short_attack_atk_rate);
      if (wd.flag & BF_LONG &&
          (skill_id != RA_WUGBITE &&
           skill_id != RA_WUGSTRIKE)) // Long damage rate addition doesn't use
                                      // weapon + equip attack
    ...
    if (wd.damage + wd.damage2) { // Check if attack ignores DEF
      if (!attack_ignores_def(&wd, src, target, skill_id, skill_lv,
                              EQI_HAND_L) ||
          !attack_ignores_def(&wd, src, target, skill_id, skill_lv, EQI_HAND_R))
...
#ifdef RENEWAL
  if (is_attack_critical(&wd, src, target, skill_id, skill_lv, false)) {
    if (sd) { // Check for player so we don't crash out, monsters don't have
              // bonus crit rates [helvetica]
      wd.damage = (int64)floor(
          (float)((wd.damage * 140) / 100 * (100 + sd->bonus.crit_atk_rate)) /
          100);
      if (is_attack_left_handed(src, skill_id))
        wd.damage2 = (int64)floor((float)((wd.damage2 * 140) / 100 *
                                          (100 + sd->bonus.crit_atk_rate)) /
                                  100);
      // if (skill_id > 0)
      // 	wd.damage = (int64)floor((float)((wd.damage * 140) / 100 * (100
      // + (sd->bonus.crit_atk_rate / 2))) / 100); else 	wd.damage =
      // (int64)floor((float)((wd.damage * 140) / 100 * (100 +
  ...
  case SR_TIGERCANNON:
    ...
    if (wd.miscflag & 8) {
      ATK_ADD(wd.damage, wd.damage2,
              skill_lv * 500 + status_get_lv(target) * 40);
    } else
      ATK_ADD(wd.damage, wd.damage2,
              skill_lv * 240 + status_get_lv(target) * 40);
    break;
  case SR_GATEOFHELL: {
    ...
    if (sc && sc->data[SC_COMBO] &&
      ...
      ATK_ADD(wd.damage, wd.damage2,
              static_cast<int64>(sstatus->max_sp * bonus) +
                  40 * status_get_lv(src));
    } else
      ATK_ADD(wd.damage, wd.damage2,
              static_cast<int64>(sstatus->sp * bonus) +
                  10 * status_get_lv(src));
  } break;
  case MH_TINDER_BREAKER:
    ATK_ADD(wd.damage, wd.damage2,
            2500 * skill_lv +
                status_get_lv(src)); // !TODO: Confirm base level bonus
    break;
  case MH_CBC:
    ATK_ADD(wd.damage, wd.damage2,
            4000 * skill_lv +
  ...
  if (sd) {
    ...
    if (skill_id != CR_SHIELDBOOMERANG) // Only Shield boomerang doesn't takes
                                        // the Star Crumbs bonus.
      ATK_ADD2(wd.damage, wd.damage2,
               ((wd.div_ < 1) ? 1 : wd.div_) * sd->right_weapon.star,
               ((wd.div_ < 1) ? 1 : wd.div_) * sd->left_weapon.star);
    if (skill_id != MC_CARTREVOLUTION && pc_checkskill(sd, BS_HILTBINDING) > 0)
    ...
    if (skill_id == CR_SHIELDBOOMERANG ||
        skill_id ==
            PA_SHIELDCHAIN) { // Refine bonus applies after cards and elements.
      short index = sd->equip_index[EQI_HAND_L];

      if (index >= 0 && sd->inventory_data[index] &&
          sd->inventory_data[index]->type == IT_ARMOR)
        ATK_ADD(wd.damage, wd.damage2,
                10 * sd->inventory.u.items_inventory[index].refine);
    }
#ifndef RENEWAL
    // Card Fix for attacker (sd), 2 is added to the "left" flag meaning
    // "attacker cards only"
    ...
    case RK_DRAGONBREATH:
    case RK_DRAGONBREATH_WATER:
      if (wd.flag & BF_LONG) { // Add check here, because we want to apply the
                               // same behavior in pre-renewal [exneval]
        wd.damage = wd.damage * (100 + sd->bonus.long_attack_atk_rate) / 100;
        if (is_attack_left_handed(src, skill_id))
          wd.damage2 =
              wd.damage2 * (100 + sd->bonus.long_attack_atk_rate) / 100;
      }
      break;
    default:
      wd.damage +=
          battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                              left_element, wd.damage, 2, wd.flag);
      if (is_attack_left_handed(src, skill_id))
        wd.damage2 +=
            battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                                left_element, wd.damage2, 3, wd.flag);
      break;
    }
#endif
  }
...
#ifndef RENEWAL
  if (tsd) { // Card Fix for target (tsd), 2 is not added to the "left" flag
             // meaning "target cards only"
    wd.damage += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                                     left_element, wd.damage, 0, wd.flag);
    if (is_attack_left_handed(src, skill_id))
      wd.damage2 +=
          battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
                              left_element, wd.damage2, 1, wd.flag);
  }
#endif
  ...
  battle_do_reflect(BF_WEAPON, &wd, src, target, skill_id,
                    skill_lv); // WIP [lighta]
...
struct Damage battle_calc_magic_attack(struct block_list *src,
  ...
  if (s_ele == ELE_WEAPON) { // pl=-1 : the skill takes the weapon's element
    ...
    if (sd && sd->spiritcharm_type != CHARM_TYPE_NONE &&
        sd->spiritcharm >= MAX_SPIRITCHARM)
      s_ele = sd->spiritcharm_type; // Summoning 10 spiritcharm will endow your
                                    // weapon
  ...
  case SO_PSYCHIC_WAVE:
    // if (sd && (sd->weapontype1 == W_STAFF || sd->weapontype1 == W_2HSTAFF ||
    // sd->weapontype1 == W_BOOK)) 	ad.div_ = 2;
    if (sc && sc->count) {
...
#endif
// MATK_RATE scales the damage. 100 = no change. 50 is halved, 200 is doubled,
// etc
#define MATK_RATE(a)                                                           \
...
#define MATK_ADD(a)                                                            \
  { ad.damage += a; }
    ...
    case NPC_DARKBREATH:
      ad.damage =
          tstatus->hp * (skill_lv <= 5 ? 100 / (2 * 6 - skill_lv) : 50) / 100;
      break;
    case PF_SOULBURN:
    ...
    case NPC_EARTHQUAKE:
      ...
      if (src->type == BL_PC)
        ad.damage =
            sstatus->str * 2 +
            battle_calc_weapon_attack(src, target, skill_id, skill_lv, mflag)
                .damage;
      else
        ad.damage = battle_calc_base_damage(src, sstatus, &sstatus->rhw, sc,
                                            tstatus->size, 0);

      MATK_RATE(200 + 100 * skill_lv + 100 * (skill_lv / 2) +
                ((skill_lv > 4) ? 100 : 0));
    ...
    default: {
      ...
      case MG_FIREBALL:
        ...
        if (ad.miscflag ==
            2) // Enemies at the edge of the area will take 75% of the damage
      ...
      case MG_LIGHTNINGBOLT:
        if (sc && sc->data[SC_SPELLFIST] && mflag & BF_SHORT) {
          skillratio += (sc->data[SC_SPELLFIST]->val4 * 100) +
                        (sc->data[SC_SPELLFIST]->val1 * 50) -
                        100; // val4 = used bolt level, val2 = used spellfist
                             // level. [Rytech]
          // skillratio += (sc->data[SC_SPELLFIST]->val3 * 100) +
          // (sc->data[SC_SPELLFIST]->val1 * 50 - 50) - 100; // val3 = used bolt
          // level, val1 = used spellfist level. [Rytech]
          ad.div_ =
              1; // ad mods, to make it work similar to regular hits [Xazax]
      ...
      case AL_HOLYLIGHT:
        skillratio += 25;
        if (sd && sd->sc.data[SC_SPIRIT] &&
            sd->sc.data[SC_SPIRIT]->val2 == SL_PRIEST)
      ...
      case NJ_KOUENKA:
        skillratio -= 10;
        if (sd && sd->spiritcharm_type == CHARM_TYPE_FIRE &&
            sd->spiritcharm > 0)
...
#endif
        if (sd && sd->spiritcharm_type == CHARM_TYPE_WATER &&
            sd->spiritcharm > 0)
          skillratio += 20 * sd->spiritcharm;
        break;
      ...
      case NJ_RAIGEKISAI:
        ...
        skillratio += 60 + 40 * skill_lv;
#endif
        if (sd && sd->spiritcharm_type == CHARM_TYPE_WIND &&
            sd->spiritcharm > 0)
          skillratio += 20 * sd->spiritcharm;
        break;
      case NJ_KAMAITACHI:
      ...
      case WL_SOULEXPANSION:
        ...
        // skillratio += -100 + 1000 + skill_lv * 200 + sstatus->int_ / 6; //
        // !TODO: Confirm INT bonus
        RE_LVL_DMOD(100);
        break;
      case WL_FROSTMISTY:
      ...
      case WL_SUMMON_ATK_GROUND:
        skillratio +=
            -100 + (1 + skill_lv) / 2 *
                       (status_get_lv(src) + (sd ? sd->status.job_level : 0));
      ...
      case LG_RAYOFGENESIS:
        ...
        RE_LVL_DMOD(100);
        break;
      case LG_SHIELDSPELL: // [(Casters Base Level x 4) + (Shield MDEF x 100) +
                           // (Casters INT x 2)] %
        if (sd && skill_lv == 2)
          skillratio += -100 + status_get_lv(src) * 4 +
                        sd->bonus.shieldmdef * 100 + status_get_int(src) * 2;
        else
          skillratio = 0;
        break;
      case WM_METALICSOUND:
        skillratio += -100 + 120 * skill_lv +
                      60 * ((sd) ? pc_checkskill(sd, WM_LESSON) : 1);
        if (tsc && tsc->data[SC_SLEEP])
      ...
      case SO_EARTHGRAVE: // !TODO: Confirm formula
        skillratio += -100 + sstatus->int_ * skill_lv +
                      ((sd) ? pc_checkskill(sd, SA_SEISMICWEAPON) * 200 : 0);
        // skillratio += -100 + sstatus->int_ / 6 * skill_lv + ((sd) ?
        // pc_checkskill(sd, SA_SEISMICWEAPON) * 200 : 0);
        RE_LVL_DMOD(100);
      ...
      case SO_SANDDUST:
        ...
        skillratio = (1000 * skill_lv + sstatus->int_ * 2 +
                      ((sd) ? pc_checkskill(sd, SA_ABRACADABRA) : 0) * 100);
        break;
      case SO_DIAMONDDUST: // !TODO: Confirm formula
        skillratio = (200 * ((sd) ? pc_checkskill(sd, SA_FROSTWEAPON) : 0) +
                      sstatus->int_ * skill_lv);
        // skillratio += -100 + 200 * ((sd) ? pc_checkskill(sd, SA_FROSTWEAPON)
        // : 0) + sstatus->int_ / 6 * skill_lv;
        RE_LVL_DMOD(100);
      ...
      case SO_POISON_BUSTER:
        skillratio += -100 + 1000 + 300 * skill_lv +
                      sstatus->int_ / 6; // !TODO: Confirm INT bonus
      ...
      case SO_PSYCHIC_WAVE:
        ...
        RE_LVL_DMOD(100);
        if (sc &&
            (sc->data[SC_HEATER_OPTION] || sc->data[SC_COOLER_OPTION] ||
      ...
      case NPC_CLOUD_KILL:
        skillratio += -100 + 50 * skill_lv;
        break;
      case SO_VARETYR_SPEAR: // MATK [{( Endow Tornado skill level x 50 ) + (
                             // Caster INT x Varetyr Spear Skill level )} x
                             // Caster Base Level / 100 ] %
        skillratio += -100 + status_get_int(src) * skill_lv +
                      ((sd) ? pc_checkskill(sd, SA_LIGHTNINGLOADER) * 50 : 0);
        RE_LVL_DMOD(100);
        if (sc && sc->data[SC_BLAST_OPTION])
          skillratio += (sd ? sd->status.job_level * 5 : 0);
        break;
      case GN_DEMONIC_FIRE:
        if (skill_lv > 20) // Fire expansion Lv.2
          ...
          skillratio += 10 + 20 * (skill_lv - 10) + status_get_int(src) +
      ...
      case MH_XENO_SLASHER:
        skillratio +=
            -100 + 450 * skill_lv * status_get_lv(src) / 100 +
            sstatus->int_ / 6; // !TODO: Confirm Base Level and INT bonus
        break;
      ...
      case MH_POISON_MIST:
        skillratio += -100 + 200 * skill_lv * status_get_lv(src) / 100 +
                      sstatus->dex / 6; // ! TODO: Confirm DEX bonus
        break;
      ...
      case NPC_VENOMFOG:
        ...
        break;
      case NPC_COMET:
        i = (sc ? distance_xy(target->x, target->y, sc->comet_x, sc->comet_y)
                : 8) /
            2;
        i = cap_value(i, 1, 4);
      ...
      if (sc) { // Insignia's increases the damage of offensive magic by a fixed
                // percentage depending on the element.
        if ((sc->data[SC_FIRE_INSIGNIA] &&
             sc->data[SC_FIRE_INSIGNIA]->val1 == 3 && s_ele == ELE_FIRE) ||
            (sc->data[SC_WATER_INSIGNIA] &&
             sc->data[SC_WATER_INSIGNIA]->val1 == 3 && s_ele == ELE_WATER) ||
            (sc->data[SC_WIND_INSIGNIA] &&
             sc->data[SC_WIND_INSIGNIA]->val1 == 3 && s_ele == ELE_WIND) ||
            (sc->data[SC_EARTH_INSIGNIA] &&
             sc->data[SC_EARTH_INSIGNIA]->val1 == 3 && s_ele == ELE_EARTH))
          skillratio += 25;
      }
...
#ifdef RENEWAL
    ad.damage += battle_calc_cardfix(BF_MAGIC, src, target, nk, s_ele, 0,
                                     ad.damage, 0, ad.flag);
#endif
    ...
    if (sd) {
      ...
      if (!flag.imdef &&
          (sd->bonus.ignore_mdef_ele & (1 << tstatus->def_ele) ||
           sd->bonus.ignore_mdef_ele & (1 << ELE_ALL) ||
           sd->bonus.ignore_mdef_race & (1 << tstatus->race) ||
           sd->bonus.ignore_mdef_race & (1 << RC_ALL) ||
    ...
    if (!nk[NK_IGNOREELEMENT])
      ad.damage = battle_attr_fix(src, target, ad.damage, s_ele,
                                  tstatus->def_ele, tstatus->ele_lv);
    ...
    case NPC_GRANDDARKNESS: {
      ...
      ad.damage = battle_attr_fix(src, target, wd.damage + ad.damage, s_ele,
                                  tstatus->def_ele, tstatus->ele_lv) *
                  (100 + 40 * skill_lv) / 100;
#ifdef RENEWAL
      if (src == target)
  ...
  ad.damage =
      battle_calc_damage(src, target, &ad, ad.damage, skill_id, skill_lv);
  if (mapdata_flag_gvg2(mapdata))
    ad.damage =
        battle_calc_gvg_damage(src, target, ad.damage, skill_id, ad.flag);
  else if (mapdata->flag[MF_BATTLEGROUND])
    ad.damage =
        battle_calc_bg_damage(src, target, ad.damage, skill_id, ad.flag);
  if (battle_config.atk_adjustment_map)
    ad.damage = battle_calc_damage_adjustment(
        src, ad.damage, ad.flag); // Global damage adjustment [Cydh]
  ...
  // [lighta] Magic skill has own handler at skill_attack
  return ad;
}
...
struct Damage battle_calc_misc_attack(struct block_list *src,
  ...
  // Some initial values
  md.amotion =
      (skill_get_inf(skill_id) & INF_GROUND_SKILL ? 0 : sstatus->amotion);
  ...
  s_ele = skill_get_ele(skill_id, skill_lv);
  if (s_ele == ELE_WEAPON ||
      s_ele == ELE_ENDOWED) // Attack that takes weapon's element for misc
                            // attacks? Make it neutral [Skotlex]
    s_ele = ELE_NEUTRAL;
  else if (s_ele == ELE_RANDOM) // Use random element
    s_ele = rnd() % ELE_ALL;
  ...
  case HT_CLAYMORETRAP:
    md.damage = (int64)(skill_lv * sstatus->dex *
                        (3.0 + (float)status_get_lv(src) / 100.0) *
  ...
  case HT_BLASTMINE:
    md.damage =
        skill_lv * (sstatus->dex / 2 + 50) * (100 + sstatus->int_) / 100;
    break;
  case HT_CLAYMORETRAP:
    md.damage =
        skill_lv * (sstatus->dex / 2 + 75) * (100 + sstatus->int_) / 100;
    break;
#endif
  case HT_BLITZBEAT:
  ...
  case ASC_BREAKER:
    ...
    nk.set(NK_IGNOREELEMENT); // These two are not properties of the weapon
                              // based part.
    break;
  case HW_GRAVITATION:
  ...
  case GN_FIRE_EXPANSION_ACID:
    ...
    // resistances
    {
      ...
      struct Damage matk =
          battle_calc_magic_attack(src, target, skill_id, skill_lv, 0);
      md.damage = 7 * ((atk.damage / skill_lv + matk.damage / skill_lv) *
                       tstatus->vit / 100);

      // AD benefits from endow/element but damage is forced back to neutral
      md.damage = battle_attr_fix(src, target, md.damage, ELE_NEUTRAL,
                                  tstatus->def_ele, tstatus->ele_lv);
    }
    // Fall through
#else
  case CR_ACIDDEMONSTRATION:
    if (tstatus->vit + sstatus->int_) // crash fix
      md.damage = (int)((int64)7 * tstatus->vit * sstatus->int_ *
                        sstatus->int_ / (10 * (tstatus->vit + sstatus->int_)));
    else
      md.damage = 0;
    if (tsd)
      md.damage >>= 1;
  ...
  case NJ_ISSEN:
    ...
    // final damage = base damage + ((mirror image count + 1) / 5 * base damage)
    // - (edef + sdef) modified def formula
    {
      short totaldef;
      struct Damage atk =
          battle_calc_weapon_attack(src, target, skill_id, skill_lv, 0);
      ...
      md.damage =
          (int64)sstatus->hp +
          (atk.damage * (int64)sstatus->hp * skill_lv) / (int64)sstatus->max_hp;

      if (sc && sc->data[SC_BUNSINJYUTSU] &&
          (i = sc->data[SC_BUNSINJYUTSU]->val2) >
              0) {          // mirror image bonus only occurs if active
  ...
  // case RA_ICEBOUNDTRAP:
    ...
    if (sd) {
      ...
      if (researchskill_lv)
        md.damage = md.damage * 20 * researchskill_lv /
                    (skill_id == RA_CLUSTERBOMB ? 50 : 100);
      else
        md.damage = 0;
    } else
  ...
  case GN_HELLS_PLANT_ATK:
    ...
    md.damage = skill_lv * status_get_lv(src) * 10 +
                status_get_int(src) * 7 / 2 *
                    ...
                    (10 - (sd ? pc_checkskill(sd, AM_CANNIBALIZE) : 0));
    break;
  case RL_B_TRAP:
    // kRO 2014-02-12: Damage: Caster's DEX, Target's current HP, Skill Level
    md.damage =
        status_get_dex(src) * 10 + (skill_lv * 3 * status_get_hp(target)) / 100;
  ...
  case SJ_NOVAEXPLOSING:
    ...
    md.damage =
        (sstatus->batk + sstatus->rhw.atk) * (200 + 100 * skill_lv) / 100;

    // Additional Damage
    md.damage += sstatus->max_hp / (6 - min(5, skill_lv)) +
                 status_get_max_sp(src) * (2 * skill_lv);
    break;
  }
  ...
  if (!nk[NK_IGNOREFLEE]) {
    ...
    if (sc && sc->opt1 && sc->opt1 != OPT1_STONEWAIT &&
      ...
      if (battle_config.agi_penalty_type &&
        ...
        if (attacker_count >= battle_config.agi_penalty_count) {
          if (battle_config.agi_penalty_type == 1)
            flee = (flee * (100 - (attacker_count -
                                   (battle_config.agi_penalty_count - 1)) *
                                      battle_config.agi_penalty_num)) /
                   100;
          else // assume type 2: absolute reduction
            flee -= (attacker_count - (battle_config.agi_penalty_count - 1)) *
      ...
      hitrate += sstatus->hit - flee;
#ifdef RENEWAL
      if (sd) // in Renewal hit bonus from Vultures Eye is not shown anymore in
              // status window
        hitrate += pc_checkskill(sd, AC_VULTURE);
#endif
      hitrate = cap_value(hitrate, battle_config.min_hitrate,
  ...
  md.damage += battle_calc_cardfix(BF_MISC, src, target, nk, s_ele, 0,
                                   md.damage, 0, md.flag);
  ...
  if (!nk[NK_IGNOREELEMENT])
    md.damage = battle_attr_fix(src, target, md.damage, s_ele, tstatus->def_ele,
                                tstatus->ele_lv);
  ...
  md.damage =
      battle_calc_damage(src, target, &md, md.damage, skill_id, skill_lv);
  if (mapdata_flag_gvg2(mapdata))
    md.damage =
        battle_calc_gvg_damage(src, target, md.damage, skill_id, md.flag);
  else if (mapdata->flag[MF_BATTLEGROUND])
    md.damage =
        battle_calc_bg_damage(src, target, md.damage, skill_id, md.flag);

  if (battle_config.atk_adjustment_map)
    md.damage = battle_calc_damage_adjustment(
        src, md.damage, md.flag); // Global damage adjustment [Cydh]
  ...
  battle_do_reflect(BF_MISC, &md, src, target, skill_id,
                    skill_lv); // WIP [lighta]
 ...
 */
void battle_vanish_damage(struct map_session_data *sd,
                          struct block_list *target, int flag) {
  nullpo_retv(sd);
  nullpo_retv(target);
  ...
  if (vanish_hp > 0 || vanish_sp > 0)
    status_percent_damage(&sd->bl, target, -vanish_hp, -vanish_sp,
                          false); // Damage HP/SP applied once
}
 ...
 *	Refined and optimized by helvetica
 */
struct Damage battle_calc_attack(int attack_type, struct block_list *bl,
                                 struct block_list *target, uint16 skill_id,
                                 uint16 skill_lv, int flag) {
  struct Damage d;
  ...
  case BF_MISC:
    ...
    break;
  default:
    ShowError("battle_calc_attack: unknown attack type! %d (skill_id=%d, "
              "skill_lv=%d)\n",
  ...
  if (d.damage + d.damage2 < 1) { // Miss/Absorbed
    ...
    if (d.dmg_lv ==
        ATK_DEF /*&& attack_type&(BF_MAGIC|BF_MISC)*/) // Isn't it that
                                                       // additional effects
                                                       // don't apply if miss?
      d.dmg_lv = ATK_MISS;
    d.dmotion = 0;
  } else // Some skills like Weaponry Research will cause damage even if attack
         // is dodged
    d.dmg_lv = ATK_DEF;
 ...
 *	Refined and optimized by helvetica
 */
int64 battle_calc_return_damage(struct block_list *bl, struct block_list *src,
                                int64 *dmg, int flag, uint16 skill_id,
                                bool status_reflect) {
  struct map_session_data *sd;
  ...
  if (sc) { // These statuses do not reflect any damage (off the target)
    if (sc->data[SC_WHITEIMPRISON] ||
        (sc->data[SC_KYOMU] && (!ssc || !ssc->data[SC_SHIELDSPELL_DEF])))
      return 0;
  }
  ...
  if (flag & BF_SHORT) { // Bounces back part of the damage.
    if ((skill_get_inf2(skill_id, INF2_ISTRAP) || !status_reflect) && sd &&
        sd->bonus.short_weapon_damage_return) {
      ...
      if (sc->data[SC_REFLECTSHIELD]) {
        ...
        if ((sce_d = sc->data[SC_DEVOTION]) &&
            (d_bl = map_id2bl(sce_d->val1)) &&
            ((d_bl->type == BL_MER && ((TBL_MER *)d_bl)->master &&
              ((TBL_MER *)d_bl)->master->bl.id == bl->id) ||
             (d_bl->type == BL_PC &&
      ...
      if (sc->data[SC_DEATHBOUND] && skill_id != WS_CARTTERMINATION &&
          skill_id != GN_HELLS_PLANT_ATK &&
          !status_bl_has_mode(src, MD_STATUSIMMUNE)) {
        if (distance_bl(src, bl) <= 0 ||
            !map_check_dir(map_calc_dir(bl, src->x, src->y), unit_getdir(bl))) {
          int64 rd1 = min(damage, status_get_max_hp(bl)) *
                      sc->data[SC_DEATHBOUND]->val2 / 100; // Amplify damage.

          *dmg = rd1 * 30 / 100; // Received damage = 30% of amplified damage.
          clif_skill_damage(src, bl, gettick(), status_get_amotion(src), 0,
                            -30000, 1, RK_DEATHBOUND,
                            sc->data[SC_DEATHBOUND]->val1, DMG_SINGLE);
          skill_blown(bl, src, skill_get_blewcount(RK_DEATHBOUND, 1),
                      unit_getdir(src), BLOWN_NONE);
          status_change_end(bl, SC_DEATHBOUND, INVALID_TIMER);
          rdamage +=
              rd1 * 70 /
              100; // Target receives 70% of the amplified damage. [Rytech]
        }
      }

      if (sc->data[SC_SHIELDSPELL_DEF] &&
          sc->data[SC_SHIELDSPELL_DEF]->val1 == 2 &&
          !status_bl_has_mode(src, MD_STATUSIMMUNE)) {
 ...
 */
bool battle_vellum_damage(struct map_session_data *sd,
                          struct block_list *target, struct Damage *wd) {
  ...
  struct status_data *tstatus = status_get_status_data(target);
  // bHPVanishRaceRate
  int16 vellum_rate_hp = cap_value(sd->hp_vanish_race[tstatus->race].rate +
                                       sd->hp_vanish_race[RC_ALL].rate,
                                   0, INT16_MAX);
  int8 vellum_hp = cap_value(sd->hp_vanish_race[tstatus->race].per +
                                 sd->hp_vanish_race[RC_ALL].per,
                             INT8_MIN, INT8_MAX);
  // bSPVanishRaceRate
  int16 vellum_rate_sp = cap_value(sd->sp_vanish_race[tstatus->race].rate +
                                       sd->sp_vanish_race[RC_ALL].rate,
                                   0, INT16_MAX);
  int8 vellum_sp = cap_value(sd->sp_vanish_race[tstatus->race].per +
                                 sd->sp_vanish_race[RC_ALL].per,
                             INT8_MIN, INT8_MAX);

  // The HP and SP damage bonus from these items don't stack because of the
  // special damage display for SP. Vellum damage overrides any other damage
  // done as well.
  if (vellum_hp && vellum_rate_hp &&
      (vellum_rate_hp >= 1000 || rnd() % 1000 < vellum_rate_hp)) {
    wd->damage = apply_rate(tstatus->max_hp, vellum_hp);
    wd->damage2 = 0;
  } else if (vellum_sp && vellum_rate_sp &&
             (vellum_rate_sp >= 1000 || rnd() % 1000 < vellum_rate_sp)) {
 ...
 *-------------------------------------------*/
void battle_drain(struct map_session_data *sd, struct block_list *tbl,
                  int64 rdamage, int64 ldamage, int race, int class_) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (i == 1 || i == 3) {
      ...
      hp += battle_calc_drain(*damage, wd->hp_drain_rate.rate,
                              wd->hp_drain_rate.per);

      sp = wd->sp_drain_class[class_] + wd->sp_drain_class[CLASS_ALL];
      sp += battle_calc_drain(*damage, wd->sp_drain_rate.rate,
                              wd->sp_drain_rate.per);
...
int battle_damage_area(struct block_list *bl, va_list ap) {
  ...
  if (status_bl_has_mode(bl, MD_SKILLIMMUNE) ||
      status_get_class(bl) == MOBID_EMPERIUM)
    return 0;
  if (bl != src && battle_check_target(src, bl, BCT_ENEMY) > 0) {
    map_freeblock_lock();
    if (src->type == BL_PC)
      battle_drain((TBL_PC *)src, bl, damage, damage, status_get_race(bl),
                   status_get_class_(bl));
    if (amotion)
      battle_delay_damage(tick, amotion, src, bl, 0, CR_REFLECTSHIELD, 0,
                          damage, ATK_DEF, 0, true, false);
    else
      status_fix_damage(src, bl, damage, 0, LG_REFLECTDAMAGE);
    clif_damage(bl, bl, tick, amotion, dmotion, damage, 1, DMG_ENDURE, 0,
                false);
    skill_additional_effect(src, bl, CR_REFLECTSHIELD, 1,
                            BF_WEAPON | BF_SHORT | BF_NORMAL, ATK_DEF, tick);
    map_freeblock_unlock();
  }
 ...
 *------------------------------------------*/
enum damage_lv battle_weapon_attack(struct block_list *src,
                                    struct block_list *target, t_tick tick,
                                    int flag) {
  ...
  if (sc &&
      !sc->count) // Avoid sc checks when there's none to check for. [Skotlex]
    sc = NULL;
  if (tsc && !tsc->count)
    tsc = NULL;
  ...
  if (sc && sc->count) {
    ...
    else if (sc->data[SC_CLOAKINGEXCEED] &&
             !(sc->data[SC_CLOAKINGEXCEED]->val4 & 2))
  ...
  if (tsc && tsc->data[SC_AUTOCOUNTER] &&
    ...
    if (dist <= 0 ||
        (!map_check_dir(dir, t_dir) && dist <= tstatus->rhw.range + 1)) {
  ...
  if (tsc && tsc->data[SC_BLADESTOP_WAIT] &&
    ...
    if (sc_start4(src, src, SC_BLADESTOP, 100,
                  sd ? pc_checkskill(sd, MO_BLADESTOP) : 5, 0, 0, target->id,
                  duration)) { // Target locked.
      clif_damage(src, target, tick, sstatus->amotion, 1, 0, 1, DMG_NORMAL, 0,
                  false); // Display MISS.
      clif_bladestop(target, src->id, 1);
      sc_start4(src, target, SC_BLADESTOP, 100, skill_lv, 0, 0, src->id,
                duration);
      return ATK_BLOCK;
    }
  }
  ...
  if (sd && (skillv = pc_checkskill(sd, MO_TRIPLEATTACK)) > 0) {
    ...
    if (sc && sc->data[SC_SKILLRATE_UP] &&
        sc->data[SC_SKILLRATE_UP]->val1 == MO_TRIPLEATTACK) {
      triple_rate += triple_rate * (sc->data[SC_SKILLRATE_UP]->val2) / 100;
      status_change_end(src, SC_SKILLRATE_UP, INVALID_TIMER);
    }
    if (rnd() % 100 < triple_rate) {
      // Need to apply canact_tick here because it doesn't go through
      // skill_castend_id
      sd->ud.canact_tick =
          i64max(tick + skill_delayfix(src, MO_TRIPLEATTACK, skillv),
                 sd->ud.canact_tick);
      if (skill_attack(BF_WEAPON, src, src, target, MO_TRIPLEATTACK, skillv,
                       tick, 0))
        return ATK_DEF;
      return ATK_MISS;
    }
  ...
  if (sc) {
    if (sc->data[SC_SACRIFICE]) {
      ...
      /**
       * We need to calculate the DMG before the hp reduction, because it can
       * kill the source. For further information: bugreport:4950
       */
      ret_val = (damage_lv)skill_attack(BF_WEAPON, src, src, target,
                                        PA_SACRIFICE, skill_lv, tick, 0);

      status_zap(src, sstatus->max_hp * 9 / 100,
                 0); // Damage to self is always 9%
      if (ret_val == ATK_NONE)
        return ATK_MISS;
      return ret_val;
    }
    if (sc->data[SC_MAGICALATTACK]) {
      if (skill_attack(BF_MAGIC, src, src, target, NPC_MAGICALATTACK,
                       sc->data[SC_MAGICALATTACK]->val1, tick, 0))
        return ATK_DEF;
      return ATK_MISS;
    }
    if (sc->data[SC_GT_ENERGYGAIN]) {
      ...
      if (sd && rnd() % 100 < sc->data[SC_GT_ENERGYGAIN]->val2)
        pc_addspiritball(sd,
                         skill_get_time2(SR_GENTLETOUCH_ENERGYGAIN,
  ...
  if (tsc && tsc->data[SC_MTF_MLEATKED] &&
      rnd() % 100 < tsc->data[SC_MTF_MLEATKED]->val2)
    clif_skill_nodamage(
        target, target, SM_ENDURE, tsc->data[SC_MTF_MLEATKED]->val1,
        sc_start(src, target, SC_ENDURE, 100, tsc->data[SC_MTF_MLEATKED]->val1,
                 skill_get_time(SM_ENDURE, tsc->data[SC_MTF_MLEATKED]->val1)));

  if (tsc && tsc->data[SC_KAAHI] && tstatus->hp < tstatus->max_hp &&
      status_charge(target, 0, tsc->data[SC_KAAHI]->val3)) {
  ...
  if (sc && sc->count) {
      ...
      if (status_charge(src, 0, 20)) {
        if (!is_infinite_defense(target, wd.flag)) {
          struct Damage ad = battle_calc_attack(
              BF_MAGIC, src, target, sc->data[SC_SPELLFIST]->val3,
              sc->data[SC_SPELLFIST]->val4, flag | BF_SHORT);

          wd.damage = ad.damage;
          DAMAGE_DIV_FIX(wd.damage,
                         wd.div_); // Double the damage for multiple hits.
        } else {
          wd.damage = 1;
    ...
    if (sc->data[SC_GIANTGROWTH] && (wd.flag & BF_SHORT) &&
        rnd() % 100 < sc->data[SC_GIANTGROWTH]->val2 &&
        !is_infinite_defense(target, wd.flag) && !vellum_damage) {
      // wd.damage += wd.damage * 150 / 100; // 2.5 times damage
      wd.damage <<= 1; // Double Damage
      skill_break_equip(
          src, src, EQP_WEAPON, 10,
          BCT_SELF); // Break chance happens on successful damage increase
    }

    if (sd && battle_config.arrow_decrement && sc->data[SC_FEARBREEZE] &&
        sc->data[SC_FEARBREEZE]->val4 > 0) {
      short idx = sd->equip_index[EQI_AMMO];
      if (idx >= 0 && sd->inventory.u.items_inventory[idx].amount >=
                          sc->data[SC_FEARBREEZE]->val4) {
        pc_delitem(sd, idx, sc->data[SC_FEARBREEZE]->val4, 0, 1,
                   LOG_TYPE_CONSUME);
        sc->data[SC_FEARBREEZE]->val4 = 0;
      }
    }
  ...
  if (damage > 0 && src != target) {
    if (sc && sc->data[SC_DUPLELIGHT] && (wd.flag & BF_SHORT) &&
        rnd() % 100 <=
            10 + 2 * sc->data[SC_DUPLELIGHT]
                         ->val1) { // Activates it only from melee damage
      uint16 skill_id;
      if (rnd() % 2 == 1)
        skill_id = AB_DUPLELIGHT_MELEE;
      else
        skill_id = AB_DUPLELIGHT_MAGIC;
      skill_attack(skill_get_type(skill_id), src, src, target, skill_id,
                   sc->data[SC_DUPLELIGHT]->val1, tick, SD_LEVEL);
    }
  }

  wd.dmotion = clif_damage(src, target, tick, wd.amotion, wd.dmotion, wd.damage,
                           wd.div_, wd.type, wd.damage2, wd.isspdamage);
  ...
  if (!(tsc && tsc->data[SC_DEVOTION]) && !vellum_damage &&
      skill_check_shadowform(target, damage, wd.div_)) {
    ...
    battle_delay_damage(tick, wd.amotion, src, target, wd.flag, 0, 0, damage,
                        wd.dmg_lv, wd.dmotion, true, wd.isspdamage);
  if (tsc) {
    if (tsc->data[SC_DEVOTION]) {
      ...
      if (d_bl &&
        ...
        if (damage > 0) {
          ...
          clif_damage(d_bl, d_bl, gettick(), wd.amotion, wd.dmotion, damage, 1,
                      DMG_NORMAL, 0, false);
    ...
    if (target->type == BL_PC && (wd.flag & BF_SHORT) &&
        tsc->data[SC_CIRCLE_OF_FIRE_OPTION]) {
      struct elemental_data *ed = ((TBL_PC *)target)->ed;

      if (ed) {
        clif_skill_damage(&ed->bl, target, tick, status_get_amotion(src), 0,
                          -30000, 1, EL_CIRCLE_OF_FIRE,
                          tsc->data[SC_CIRCLE_OF_FIRE_OPTION]->val1,
                          DMG_SINGLE);
        skill_attack(BF_WEAPON, &ed->bl, &ed->bl, src, EL_CIRCLE_OF_FIRE,
                     tsc->data[SC_CIRCLE_OF_FIRE_OPTION]->val1, tick, wd.flag);
      }
    }
    if (tsc->data[SC_WATER_SCREEN_OPTION]) {
      struct block_list *e_bl =
          map_id2bl(tsc->data[SC_WATER_SCREEN_OPTION]->val1);
  ...
  if (sc && sc->data[SC_AUTOSPELL] &&
      rnd() % 100 < sc->data[SC_AUTOSPELL]->val4) {
    int sp = 0;
    ...
    if (status_charge(src, 0, sp)) {
      ...
      case CAST_GROUND:
        skill_castend_pos2(src, target->x, target->y, skill_id, skill_lv, tick,
                           flag);
        break;
      case CAST_NODAMAGE:
  ...
  if (sd) {
    ...
    if (wd.flag & BF_WEAPON && sc && sc->data[SC__AUTOSHADOWSPELL] &&
        rnd() % 100 < sc->data[SC__AUTOSHADOWSPELL]->val3 &&
        (r_skill = (uint16)sc->data[SC__AUTOSHADOWSPELL]->val1) &&
        (sk_idx = skill_get_index(r_skill)) &&
        sd->status.skill[sk_idx].id != 0 &&
        sd->status.skill[sk_idx].flag == SKILL_FLAG_PLAGIARIZED) {
      if (r_skill != AL_HOLYLIGHT && r_skill != PR_MAGNUS) {
        ...
        if ((type = skill_get_casttype(r_skill)) == CAST_GROUND) {
          ...
          if (!(BL_PC & battle_config.skill_reiteration) &&
              skill->unit_flag[UF_NOREITERATION])
            type = -1;

          if (BL_PC & battle_config.skill_nofootset &&
              skill->unit_flag[UF_NOFOOTSET])
            type = -1;
          ...
          if (BL_PC & battle_config.land_skill_limit &&
            ...
            int v;
            for (v = 0;
                 v < MAX_SKILLUNITGROUP && sd->ud.skillunit[v] && maxcount;
                 v++) {
        ...
        if (sd->state.autocast == 0) {
          ...
          switch (type) {
          case CAST_GROUND:
            skill_castend_pos2(src, target->x, target->y, r_skill, r_lv, tick,
                               flag);
            break;
          case CAST_NODAMAGE:
        ...
        sd->ud.canact_tick = i64max(tick + skill_delayfix(src, r_skill, r_lv),
                                    sd->ud.canact_tick);
        clif_status_change(src, EFST_POSTDELAY, 1,
                           skill_delayfix(src, r_skill, r_lv), 0, 0, 1);
      }
    }
    if (wd.flag & BF_WEAPON && sc && sc->data[SC_FALLINGSTAR] &&
        rand() % 100 < sc->data[SC_FALLINGSTAR]->val2) {
      if (sd)
        sd->state.autocast = 1;
      if (status_charge(
              src, 0,
              skill_get_sp(SJ_FALLINGSTAR_ATK, sc->data[SC_FALLINGSTAR]->val1)))
        skill_castend_nodamage_id(src, src, SJ_FALLINGSTAR_ATK,
                                  sc->data[SC_FALLINGSTAR]->val1, tick, flag);
      if (sd)
        sd->state.autocast = 0;
    }
    if (wd.flag & BF_WEAPON && src != target && damage > 0) {
      if (battle_config.left_cardfix_to_right)
        battle_drain(sd, target, wd.damage, wd.damage, tstatus->race,
                     tstatus->class_);
      else
        battle_drain(sd, target, wd.damage, wd.damage2, tstatus->race,
                     tstatus->class_);
    }
  }
  ...
  if (tsc) {
    if (damage > 0 && tsc->data[SC_POISONREACT] &&
      ...
      if (sstatus->def_ele == ELE_POISON) {
        sce->val2 = 0;
        skill_attack(BF_WEAPON, target, target, src, AS_POISONREACT, sce->val1,
                     tick, 0);
      } else {
...
int battle_check_target(struct block_list *src, struct block_list *target,
  ...
  int strip_enemy = 1; // Flag which marks whether to remove the BCT_ENEMY
                       // status if it's also friend/ally.
  ...
  case BL_PC: {
    ...
    if (((TBL_PC *)target)->invincible_timer != INVALID_TIMER ||
        pc_isinvisible((TBL_PC *)target))
      return -1; // Cannot be targeted yet.
    if (sc && sc->count) {
      if (sc->data[SC_VOICEOFSIREN] &&
          sc->data[SC_VOICEOFSIREN]->val2 == target->id)
        return -1;
    }
  } break;
  case BL_MOB: {
  ...
  case BL_SKILL: {
    ...
    if (skill_get_inf2(su->group->skill_id, INF2_ISTRAP) &&
      ...
      if (!skill_id || su->group->skill_id == NPC_REVERBERATION ||
          su->group->skill_id == WM_POEMOFNETHERWORLD) {
        ;
      } else if (skill_get_inf2(
                     skill_id,
                     INF2_TARGETTRAP)) { // Only a few skills can target traps
        switch (skill_id) {
        case RK_DRAGONBREATH:
        case RK_DRAGONBREATH_WATER:
        case NC_SELFDESTRUCTION:
      ...
      case HT_CLAYMORETRAP:
        ...
        return 0;
      default:
        // Usually BCT_ALL stands for only hitting chars, but skills
        // specifically set to hit traps also hit icewall
        if ((flag & BCT_ALL) == BCT_ALL &&
            !skill_get_inf2(skill_id, INF2_TARGETTRAP))
          return -1;
      }
      state |= BCT_ENEMY;
      strip_enemy = 0;
    } else // Excepting traps, Icewall, and Wall of Thorns, you should not be
           // able to target skills.
      return 0;
  } break;
  case BL_MER:
  case BL_HOM:
  ...
  case BL_PC: {
    ...
    if (((sd->state.block_action & PCBLOCK_IMMUNE) ||
         (sc->data[SC_KINGS_GRACE] && s_bl->type != BL_PC)) &&
        flag & BCT_ENEMY)
      return 0; // Global immunity only to Attacks
    if (sd->status.karma && s_bl->type == BL_PC &&
  ...
  case BL_SKILL: {
    ...
    if (sc && sc->count && inf2[INF2_ISTRAP]) {
      if (sc->data[SC_SIGHTBLASTER] && sc->data[SC_SIGHTBLASTER]->val2 > 0 &&
          sc->data[SC_SIGHTBLASTER]->val4 % 2 == 0)
        return -1;
    }
  } break;
  case BL_MER:
    if (t_bl->type == BL_MOB && ((TBL_MOB *)t_bl)->mob_id == MOBID_EMPERIUM &&
        flag & BCT_ENEMY)
    ...
    if (s_bl != t_bl) {
      if (sd->state.killer) {
        ...
        strip_enemy = 0;
                 ...
                 !((!battle_config.duel_allow_pvp && mapdata->flag[MF_PVP]) ||
                   (!battle_config.duel_allow_gvg &&
                    mapdata_flag_gvg(mapdata)))) {
        if (t_bl->type == BL_PC &&
            (sd->duel_group == ((TBL_PC *)t_bl)->duel_group))
          return (BCT_ENEMY & flag)
                     ? 1
                     : -1; // Duel targets can ONLY be your enemy, nothing else.
        else
          return 0; // You can't target anything out of your duel
      }
    }
    if (!sd->status.guild_id && t_bl->type == BL_MOB &&
        ((TBL_MOB *)t_bl)->mob_id == MOBID_EMPERIUM &&
        mapdata_flag_gvg(mapdata))
  ...
  case BL_MOB: {
    ...
    if (!md->special_state.ai) { // Normal mobs
      if ((target->type == BL_MOB && t_bl->type == BL_PC &&
           (((TBL_MOB *)target)->special_state.ai != AI_ZANZOU &&
            ((TBL_MOB *)target)->special_state.ai != AI_ATTACK)) ||
  ...
  if ((flag & BCT_ALL) == BCT_ALL) { // All actually stands for all attackable
                                     // chars, icewall and traps
  ...
  if (mapdata_flag_vs(mapdata)) { // Check rivalry settings.
    ...
    if (flag & (BCT_PARTY | BCT_ENEMY)) {
      ...
      if (s_party && s_party == status_get_party_id(t_bl) &&
          !(mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP_NOPARTY]) &&
          !(mapdata_flag_gvg(mapdata) && mapdata->flag[MF_GVG_NOPARTY]) &&
          (!mapdata->flag[MF_BATTLEGROUND] || sbg_id == tbg_id))
        state |= BCT_PARTY;
      else
        state |= BCT_ENEMY;
    }
    if (flag & (BCT_GUILD | BCT_ENEMY)) {
      ...
      if (!(mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP_NOGUILD]) &&
          s_guild && t_guild &&
          (s_guild == t_guild ||
           (!(flag & BCT_SAMEGUILD) && guild_isallied(s_guild, t_guild))) &&
          (!mapdata->flag[MF_BATTLEGROUND] || sbg_id == tbg_id))
        state |= BCT_GUILD;
      else
        state |= BCT_ENEMY;
    }
    if (state & BCT_ENEMY && mapdata->flag[MF_BATTLEGROUND] && sbg_id &&
        sbg_id == tbg_id)
      state &= ~BCT_ENEMY;

    if (state & BCT_ENEMY && battle_config.pk_mode &&
        !mapdata_flag_gvg(mapdata) && s_bl->type == BL_PC &&
        t_bl->type == BL_PC) { // Prevent novice engagement on pk_mode (feature
                               // by Valaris)
      ...
      if ((sd->class_ & MAPID_UPPERMASK) == MAPID_NOVICE ||
          ...
          (battle_config.pk_level_range &&
           abs((int)sd->status.base_level - (int)sd2->status.base_level) >
               battle_config.pk_level_range))
        state &= ~BCT_ENEMY;
    }
  ...
  if (!state) // If not an enemy, nor a guild, nor party, nor yourself, it's
              // neutral.
    state = BCT_NEUTRAL;
 ...
 *------------------------------------------
 */
bool battle_check_range(struct block_list *src, struct block_list *bl,
                        int range) {
  int d;
  ...
  if (src->type == BL_PC) { // Range for players' attacks and skills should
                            // always have a circular check. [Angezerus]
...
static const struct _battle_data {
  ...
  int max;
    ...
    {"natural_heal_weight_rate", &battle_config.natural_heal_weight_rate, 50, 0,
     100},
    {"natural_heal_weight_rate_renewal",
     &battle_config.natural_heal_weight_rate_renewal, 70, 0, 100},
    {
        "arrow_decrement",
    ...
    {
        "atk_adjustment_map",
        &battle_config.atk_adjustment_map,
        4082,
        0,
        4095,
    },
    ...
    {"skill_amotion_leniency", &battle_config.skill_amotion_leniency, 0, 0,
     300},
    ...
    {"feature.atcommand_suggestions",
     &battle_config.atcommand_suggestions_enabled, 0, 0, 1},
...
int battle_set_value(const char *w1, const char *w2) {
  ...
  int i;
  ARR_FIND(0, ARRAYLENGTH(battle_data), i,
           strcmpi(w1, battle_data[i].str) == 0);
  if (i == ARRAYLENGTH(battle_data))
    return 0; // not found

  if (val < battle_data[i].min || val > battle_data[i].max) {
    ShowWarning("Value for setting '%s': %s is invalid (min:%i max:%i)! "
                "Defaulting to %i...\n",
                w1, w2, battle_data[i].min, battle_data[i].max,
...
void battle_adjust_conf() {
  ...
  battle_config.max_walk_speed =
      100 * DEFAULT_WALK_SPEED / battle_config.max_walk_speed;
  battle_config.max_cart_weight *= 10;

  if (battle_config.max_def > 100 &&
      !battle_config.weapon_defense_type) // added by [Skotlex]
  ...
  if (battle_config.day_duration &&
      battle_config.day_duration < 60000) // added by [Yor]
    battle_config.day_duration = 60000;
  if (battle_config.night_duration &&
      battle_config.night_duration < 60000) // added by [Yor]
    battle_config.night_duration = 60000;

#if PACKETVER < 20100427
  if (battle_config.feature_buying_store) {
    ShowWarning("conf/battle/feature.conf:buying_store is enabled but it "
                "requires PACKETVER 2010-04-27 or newer, disabling...\n");
  ...
  if (battle_config.feature_search_stores) {
    ShowWarning("conf/battle/feature.conf:search_stores is enabled but it "
                "requires PACKETVER 2010-08-03 or newer, disabling...\n");
  ...
  if (battle_config.feature_bgqueue) {
    ShowWarning("conf/battle/feature.conf:bgqueue is enabled but it requires "
                "PACKETVER 2012-01-01 or newer, disabling...\n");
    battle_config.feature_bgqueue = 0;
  }
#endif

#if PACKETVER > 20120000 &&                                                    \
    PACKETVER < 20130515 /* Exact date (when it started) not known */
  if (battle_config.feature_auction) {
    ShowWarning(
        "conf/battle/feature.conf:feature.auction is enabled but it is not "
        "stable on PACKETVER " EXPAND_AND_QUOTE(PACKETVER) ", disabling...\n");
    ShowWarning("conf/battle/feature.conf:feature.auction change value to '2' "
                "to silence this warning and maintain it enabled\n");
    battle_config.feature_auction = 0;
  }
#elif PACKETVER >= 20141112
  if (battle_config.feature_auction) {
    ShowWarning("conf/battle/feature.conf:feature.auction is enabled but it is "
                "not available for clients from 2014-11-12 on, disabling...\n");
    ShowWarning("conf/battle/feature.conf:feature.auction change value to '2' "
                "to silence this warning and maintain it enabled\n");
    battle_config.feature_auction = 0;
  }
  ...
  if (battle_config.feature_banking) {
    ShowWarning("conf/battle/feature.conf banking is enabled but it requires "
                "PACKETVER 2013-07-24 or newer, disabling...\n");
  ...
  if (battle_config.mvp_exp_reward_message) {
    ShowWarning("conf/battle/client.conf MVP EXP reward message is enabled but "
                "it requires PACKETVER 2013-12-23 or newer, disabling...\n");
  ...
  if (battle_config.feature_roulette) {
    ShowWarning("conf/battle/feature.conf roulette is enabled but it requires "
                "PACKETVER 2014-10-22 or newer, disabling...\n");
  ...
  if (battle_config.feature_achievement) {
    ShowWarning("conf/battle/feature.conf achievement is enabled but it "
                "requires PACKETVER 2015-05-13 or newer, disabling...\n");
  ...
  if (battle_config.feature_petevolution) {
    ShowWarning("conf/battle/feature.conf petevolution is enabled but it "
                "requires PACKETVER 2014-10-08 or newer, disabling...\n");
    battle_config.feature_petevolution = 0;
  }
  if (battle_config.feature_pet_autofeed) {
    ShowWarning("conf/battle/feature.conf pet auto feed is enabled but it "
                "requires PACKETVER 2014-10-08 or newer, disabling...\n");
  ...
  if (battle_config.feature_refineui) {
    ShowWarning("conf/battle/feature.conf refine UI is enabled but it requires "
                "PACKETVER 2016-10-12 or newer, disabling...\n");
  ...
  if (battle_config.feature_equipswitch) {
    ShowWarning("conf/battle/feature.conf equip switch is enabled but it "
                "requires PACKETVER 2017-02-08 or newer, disabling...\n");
  ...
  if (battle_config.feature_homunculus_autofeed) {
    ShowWarning(
        "conf/battle/feature.conf homunculus autofeeding is enabled but it "
        "requires PACKETVER 2017-09-20 or newer, disabling...\n");
  ...
  if (battle_config.feature_attendance) {
    ShowWarning("conf/battle/feature.conf attendance system is enabled but it "
                "requires PACKETVER 2018-03-07 or newer, disabling...\n");
  ...
  if (battle_config.feature_privateairship) {
    ShowWarning(
        "conf/battle/feature.conf private airship system is enabled but it "
        "requires PACKETVER 2018-03-21 or newer, disabling...\n");
  ...
  if (battle_config.custom_cell_stack_limit != 1)
    ShowWarning("Battle setting 'custom_cell_stack_limit' takes no effect as "
                "this server was compiled without Cell Stack Limit support.\n");
#endif
}
...
void do_init_battle(void) {
  delay_damage_ers = ers_new(sizeof(struct delay_damage),
                             "battle.cpp::delay_damage_ers", ERS_OPT_CLEAR);
 ...
 * end battle timer
 *------------------*/
void do_final_battle(void) { ers_destroy(delay_damage_ers); }
