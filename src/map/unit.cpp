...
const short dirx[DIR_MAX] = {
    0, -1, -1, -1,
    0, 1,  1,  1}; /// lookup to know where will move to x according dir
const short diry[DIR_MAX] = {
    1,  1,  0, -1,
    -1, -1, 0, 1}; /// lookup to know where will move to y according dir
...
int unit_walktoxy_sub(struct block_list *bl) {
  ...
  if (!path_search(&wpd, bl->m, bl->x, bl->y, ud->to_x, ud->to_y,
                   ud->state.walk_easy, CELL_CHKNOPASS))
    return 0;

#ifdef OFFICIAL_WALKPATH
  if (bl->type != BL_NPC   // If type is a NPC, please disregard.
      && wpd.path_len > 14 // Official number of walkable cells is 14 if and
                           // only if there is an obstacle between. [malufett]
      && !path_search_long(
             nullptr, bl->m, bl->x, bl->y, ud->to_x, ud->to_y,
             CELL_CHKNOPASS)) // Check if there is an obstacle between
    return 0;
#endif
  ...
  if (ud->target_to && ud->chaserange > 1) {
    ...
    for (i = (ud->chaserange * 10) - 10; i > 0 && ud->walkpath.path_len > 1;) {
      ...
      if (direction_diagonal(dir))
        i -= MOVE_COST * 20; // When chasing, units will target a diamond-shaped
                             // area in range [Playtester]
      else
        i -= MOVE_COST;
      ud->to_x -= dirx[dir];
      ud->to_y -= diry[dir];
  ...
  // If this is a walking NPC and it will use a player sprite
           ...
           pcdb_checkid(status_get_viewdata(bl)->class_)) {
...
/**
 * Warps a unit to its master if the master has gone out of sight (3 second
 * default) Can be any object with a master [MOB|PET|HOM|MER|ELEM]
 * @param tid: Timer
 ...
 * If so, starts a timer (default: 3 seconds) which will teleport the unit back
 * to master
  ...
  if (msd && max_dist) {
    ...
    if (!check_distance_bl(&msd->bl, sbl, max_dist)) {
      if (*msd_tid == INVALID_TIMER || *msd_tid == 0)
        *msd_tid =
            add_timer(gettick() + 3000, unit_teleport_timer, sbl->id, max_dist);
    } else {
...
TIMER_FUNC(unit_step_timer) {
  ...
  // Set to false here because if an error occurs, it should not be executed
  // again
  ud->stepaction = false;
  ...
  // Flush target_to as it might contain map coordinates which should not be
  // used by other functions
  target_id = ud->target_to;
  ud->target_to = 0;

  // If stepaction is set then we remembered a client request that should be
  // executed on the next step Execute request now if target is in attack range
  if (ud->stepskill_id && skill_get_inf(ud->stepskill_id) & INF_GROUND_SKILL) {
    // Execute ground skill
    struct map_data *md = &map[bl->m];
    unit_skilluse_pos(bl, target_id % md->xs, target_id / md->xs,
                      ud->stepskill_id, ud->stepskill_lv);
  } else {
  ...
  case BL_PC:
    ...
    for (size_t i = 0; i < sd->areanpc.size(); i++) {
      ...
      if (!nd || nd->subtype != NPCTYPE_SCRIPT ||
          !(nd->bl.m == bl->m && bl->x >= nd->bl.x - nd->u.scr.xs &&
            bl->x <= nd->bl.x + nd->u.scr.xs &&
            bl->y >= nd->bl.y - nd->u.scr.ys &&
            bl->y <= nd->bl.y + nd->u.scr.ys))
...
/**
 * Defines when to refresh the walking character to object and restart the timer
 * if applicable Also checks for speed update, target location, and slave
 * teleport timers
 * @param tid: Timer ID
...
static TIMER_FUNC(unit_walktoxy_timer) {
  ...
  // Get icewall walk block depending on Status Immune mode (players can't be
  // trapped)
  ...
  // Monsters will walk into an icewall from the west and south if they already
  // started walking
  if (map_getcell(bl->m, x + dx, y + dy, CELL_CHKNOPASS) &&
      (icewall_walk_block == 0 || dx < 0 || dy < 0 ||
       !map_getcell(bl->m, x + dx, y + dy, CELL_CHKICEWALL)))
    return unit_walktoxy_sub(bl);

  // Monsters can only leave icewalls to the west and south
  // But if movement fails more than icewall_walk_block times, they can ignore
  // this rule
  if (md && md->walktoxy_fail_count < icewall_walk_block &&
      map_getcell(bl->m, x, y, CELL_CHKICEWALL) && (dx > 0 || dy > 0)) {
    // Needs to be done here so that rudeattack skills are invoked
    md->walktoxy_fail_count++;
    clif_fixpos(bl);
    // Monsters in this situation first use a chase skill, then unlock target
    // and then use an idle skill
  ...
  map_foreachinmovearea(clif_outsight, bl, AREA_SIZE, dx, dy,
                        sd ? BL_ALL : BL_PC, bl);

  x += dx;
  y += dy;
  map_moveblock(bl, x, y, tick);
  ud->walk_count++; // Walked cell counter, to be used for walk-triggered
                    // skills. [Skotlex]

  if (bl->x != x || bl->y != y || ud->walktimer != INVALID_TIMER)
    return 0; // map_moveblock has altered the object beyond what we expected
              // (moved/warped it)

  ud->walktimer = CLIF_WALK_TIMER; // Arbitrary non-INVALID_TIMER value to make
                                   // the clif code send walking packets
  map_foreachinmovearea(clif_insight, bl, AREA_SIZE, -dx, -dy,
                        sd ? BL_ALL : BL_PC, bl);
  ud->walktimer = INVALID_TIMER;
  ...
  if (bl->x == ud->to_x && bl->y == ud->to_y) {
    ...
    if (ud->walk_done_event[0]) {
      ...
      // Copying is required in case someone uses unitwalkto inside the event
      // code
  ...
  case BL_PC:
    ...
    if (map_getcell(bl->m, x, y, CELL_CHKNPC)) {
      ...
      if (bl->prev == NULL) // Script could have warped char, abort remaining of
                            // the function.
        return 0;
    } else
      sd->areanpc.clear();
  ...
  case BL_MOB:
    ...
    if (md->min_chase > md->db->range3)
      md->min_chase--;
    // Walk skills are triggered regardless of target due to the idle-walk mob
    // state. But avoid triggering on stop-walk calls.
    if (tid != INVALID_TIMER && !(ud->walk_count % WALK_SKILL_INTERVAL) &&
        map[bl->m].users > 0 && mobskill_use(md, tick, -1)) {
      if (!(ud->skill_id == NPC_SELFDESTRUCTION &&
            ud->skilltimer != INVALID_TIMER) &&
          md->state.skillstate !=
              MSS_WALK)  // Walk skills are supposed to be used while walking
  ...
  if (tid ==
      INVALID_TIMER) // A directly invoked timer is from battle_stop_walking,
                     // therefore the rest is irrelevant.
    return 0;
  ...
  if (ud->stepaction && ud->target_to) {
    // Delete old stepaction even if not executed yet, the latest command is
    // what counts
  ...
  if (speed > 0) {
    ...
    if (md && DIFF_TICK(tick, md->dmgtick) <
                  3000) // Not required not damaged recently
      clif_move(ud);
    ...
    if (tbl->m == bl->m &&
        check_distance_bl(bl, tbl, ud->chaserange)) { // Reached destination.
      ...
      unit_walktobl(bl, tbl, ud->chaserange,
                    ud->state.walk_easy | (ud->state.attack_continue ? 2 : 0));

      return 0;
    }
  } else { // Stopped walking. Update to_x and to_y to current location
           // [Skotlex]
    ud->to_x = bl->x;
    ud->to_y = bl->y;

    if (battle_config.official_cell_stack_limit > 0 &&
        map_count_oncell(bl->m, x, y, BL_CHAR | BL_NPC, 1) >
...
int unit_walktoxy(struct block_list *bl, short x, short y, unsigned char flag) {
  ...
  if ((flag & 8) && !map_closest_freecell(bl->m, &x, &y, BL_CHAR | BL_NPC,
                                          1)) // This might change x and y
    return 0;

  walkpath_data wpd = {0};

  if (!path_search(&wpd, bl->m, bl->x, bl->y, x, y, flag & 1,
                   CELL_CHKNOPASS)) // Count walk path cells
    return 0;
  ...
  if (bl->type != BL_NPC) {
    ...
    // obstacle between.
    if (wpd.path_len > 14 &&
        !path_search_long(nullptr, bl->m, bl->x, bl->y, x, y, CELL_CHKNOPASS)) {
      return 0;
    }
#endif
  }
  ...
  if (flag & 4) {
    ...
    if (DIFF_TICK(ud->canmove_tick, gettick()) > 0 &&
        DIFF_TICK(ud->canmove_tick, gettick()) <
            2000) { // Delay walking command. [Skotlex]
      add_timer(ud->canmove_tick + 1, unit_delay_walktoxy_timer, bl->id,
                (x << 16) | (y & 0xFFFF));
      return 1;
    }
  }
  ...
  if (!(flag & 2) &&
      (!status_bl_has_mode(bl, MD_CANMOVE) || !unit_can_move(bl)))
    return 0;
  ...
  if (ud->walktimer != INVALID_TIMER) {
    // When you come to the center of the grid because the change of destination
    // while you're walking right now Call a function from a timer
    // unit_walktoxy_sub
 ...
 *	&2: Start attacking upon arrival within range, otherwise just walk to
 *target
 * @return 1: Started walking or set timer 0: Failed
 */
int unit_walktobl(struct block_list *bl, struct block_list *tbl, int range,
                  unsigned char flag) {
  nullpo_ret(bl);
  nullpo_ret(tbl);
  ...
  if (!unit_can_reach_bl(bl, tbl, distance_bl(bl, tbl) + 1, flag & 1, &ud->to_x,
                         &ud->to_y)) {
    ud->to_x = bl->x;
    ud->to_y = bl->y;
    ud->target_to = 0;
  ...
  if (DIFF_TICK(ud->canmove_tick, gettick()) >
      0) { // Can't move, wait a bit before invoking the movement.
 ...
 */
void unit_run_hit(struct block_list *bl, struct status_change *sc,
                  struct map_session_data *sd, enum sc_type type) {
  int lv = sc->data[type]->val1;

  // If you can't run forward, you must be next to a wall, so bounce back.
  // [Skotlex]
  if (type == SC_RUN)
    clif_status_change(bl, EFST_TING, 1, 0, 0, 0, 0);

  // Set running to 0 beforehand so status_change_end knows not to enable spurt
  // [Kevin]
  unit_bl2ud(bl)->state.running = 0;
  status_change_end(bl, type, INVALID_TIMER);

  if (type == SC_RUN) {
    skill_blown(bl, bl, skill_get_blewcount(TK_RUN, lv), unit_getdir(bl),
                BLOWN_NONE);
 ...
 * @param sd: Required only when using SC_WUGDASH
 * @return true: Success (Finished running) false: Fail (Hit an object/Couldn't
 * run)
 */
bool unit_run(struct block_list *bl, struct map_session_data *sd,
              enum sc_type type) {
  ...
  for (i = 0; i < AREA_SIZE; i++) {
    ...
    if (map_count_oncell(bl->m, to_x + dir_x, to_y + dir_y,
                         BL_PC | BL_MOB | BL_NPC, 0))
      break;
...
int unit_escape(struct block_list *bl, struct block_list *target, short dist,
  ...
  while (dist > 0 && map_getcell(bl->m, bl->x + dist * dirx[dir],
                                 bl->y + dist * diry[dir], CELL_CHKNOREACH))
    dist--;

  return (dist > 0 && unit_walktoxy(bl, bl->x + dist * dirx[dir],
                                    bl->y + dist * diry[dir], flag));
}
 ...
 *		2: Long path check (no obstacle on line from start to
 *destination)
 * @param checkpath: Whether or not to do a cell and path check for NOPASS and
 *NOREACH
 * @return True: Success False: Fail
 */
bool unit_movepos(struct block_list *bl, short dst_x, short dst_y, int easy,
                  bool checkpath) {
  short dx, dy;
  uint8 dir;
  ...
  if (checkpath && (map_getcell(bl->m, dst_x, dst_y, CELL_CHKNOPASS) ||
                    !path_search(NULL, bl->m, bl->x, bl->y, dst_x, dst_y, easy,
                                 CELL_CHKNOREACH)))
  ...
  if (sd) {
    ...
    if (sd->status.pet_id > 0 && sd->pd &&
        sd->pd->pet.intimate > PET_INTIMATE_NONE) {
      // Check if pet needs to be teleported. [Skotlex]
      int flag = 0;
      struct block_list *pbl = &sd->pd->bl;

      if (!checkpath && !path_search(NULL, pbl->m, pbl->x, pbl->y, dst_x, dst_y,
                                     0, CELL_CHKNOPASS))
        flag = 1;
      else if (!check_distance_bl(&sd->bl, pbl,
                                  AREA_SIZE)) // Too far, teleport.
        flag = 2;
 ...
 */
int unit_blown(struct block_list *bl, int dx, int dy, int count,
  ...
  if (count) {
    ...
    if (dx || dy) {
      map_foreachinmovearea(clif_outsight, bl, AREA_SIZE, dx, dy,
                            bl->type == BL_PC ? BL_ALL : BL_PC, bl);

      if (su) {
        if (su->group &&
            skill_get_unit_flag(su->group->skill_id, UF_KNOCKBACKGROUP))
...
enum e_unit_blown unit_blown_immune(struct block_list *bl, uint8 flag) {
  ...
  case BL_PC: {
    ...
    if (!(flag & 0x4) && sd->sc.data[SC_BASILICA] &&
        sd->sc.data[SC_BASILICA]->val4 == sd->bl.id)
  ...
  case BL_SKILL: {
    ...
    if (su && su->group &&
        skill_get_unit_flag(su->group->skill_id, UF_NOKNOCKBACK))
 ...
 * This function checks for "no warp" map flags, so it's safe to call without
 * doing nowarpto/nowarp checks
...
int unit_warp(struct block_list *bl, short m, short x, short y, clr_type type) {
  ...
  switch (bl->type) {
  case BL_MOB:
    if (map_getmapflag(bl->m, MF_MONSTER_NOTELEPORT) &&
        ((TBL_MOB *)bl)->master_id == 0)
      return 1;

    if (m != bl->m && map_getmapflag(m, MF_NOBRANCH) &&
        battle_config.mob_warp & 4 && !(((TBL_MOB *)bl)->master_id))
      return 1;
    break;
  case BL_PC:
  ...
  if (x < 0 || y < 0) { // Random map position.
    if (!map_search_freecell(NULL, m, &x, &y, -1, -1, 1)) {
      ShowWarning("unit_warp failed. Unit Id:%d/Type:%d, target position map "
                  "%d (%s) at [%d,%d]\n",
                  bl->id, bl->type, m, map[m].name, x, y);
      return 2;
    }
  } else if (bl->type != BL_NPC &&
             map_getcell(m, x, y, CELL_CHKNOREACH)) { // Invalid target cell
    ShowWarning(
        "unit_warp: Specified non-walkable target cell: %d (%s) at [%d,%d]\n",
        m, map[m].name, x, y);

    if (!map_search_freecell(NULL, m, &x, &y, 4, 4,
                             1)) { // Can't find a nearby cell
      ShowWarning("unit_warp failed. Unit Id:%d/Type:%d, target position map "
                  "%d (%s) at [%d,%d]\n",
                  bl->id, bl->type, m, map[m].name, x, y);
      return 2;
  ...
  if (!battle_config.slave_stick_with_master && bl->type == BL_MOB &&
      mob_countslave(bl) > 0)
 ...
 *	USW_MOVE_FULL_CELL: Enable moving to the next cell when unit was already
 *half-way there (may cause on-touch/place side-effects, such as a scripted map
 *change) USW_FORCE_STOP: Force stop moving, even if walktimer is currently
 *INVALID_TIMER
...
int unit_stop_walking(struct block_list *bl, int type) {
  ...
  if ((type & USW_MOVE_ONCE &&
       !ud->walkpath.path_pos) // Force moving at least one cell.
      || (type & USW_MOVE_FULL_CELL && td &&
          DIFF_TICK(td->tick, tick) <=
              td->data / 2) // Enough time has passed to cover half-cell
  ) {
  ...
  // Re-added, the check in unit_set_walkdelay means dmg during running won't
  // fall through to this place in code [Kevin]
  if (ud->state.running) {
 ...
 */
int unit_skilluse_id(struct block_list *src, int target_id, uint16 skill_id,
                     uint16 skill_lv) {
...
int unit_can_move(struct block_list *bl) {
  ...
  if (ud->skilltimer != INVALID_TIMER && ud->skill_id != LG_EXEEDBREAK &&
      (!sd || !pc_checkskill(sd, SA_FREECAST) ||
       skill_get_inf2(ud->skill_id, INF2_ISGUILD)))
  ...
  if ((sd &&
       (pc_issit(sd) || sd->state.vending || sd->state.buyingstore ||
        (sd->state.block_action & PCBLOCK_MOVE) || sd->state.mail_writing)) ||
      ud->state.blockedmove)
    return 0; // Can't move

  // Status changes that block movement
  if (sc) {
    if (sc->cant.move // status placed here are ones that cannot be cached by
                      // sc->cant.move for they depend on other conditions other
    ...
    if ((sc->option & OPTION_HIDE) &&
        (!sd || pc_checkskill(sd, RG_TUNNELDRIVE) <= 0))
      return 0;
  }
  ...
  if (bl->type == BL_MOB) {
    struct mob_data *md = BL_CAST(BL_MOB, bl);
    if (md && ((status_has_mode(&md->status, MD_STATUSIMMUNE) &&
                battle_config.boss_icewall_walk_block == 1 &&
                map_getcell(bl->m, bl->x, bl->y, CELL_CHKICEWALL)) ||
               (!status_has_mode(&md->status, MD_STATUSIMMUNE) &&
                battle_config.mob_icewall_walk_block == 1 &&
                map_getcell(bl->m, bl->x, bl->y, CELL_CHKICEWALL)))) {
...
TIMER_FUNC(unit_resume_running) {
  ...
  if (sd && pc_isridingwug(sd))
    clif_skill_nodamage(ud->bl, ud->bl, RA_WUGDASH, ud->skill_lv,
                        sc_start4(ud->bl, ud->bl, status_skill2sc(RA_WUGDASH),
                                  100, ud->skill_lv, unit_getdir(ud->bl), 0, 0,
                                  0));
  else
    clif_skill_nodamage(ud->bl, ud->bl, TK_RUN, ud->skill_lv,
                        sc_start4(ud->bl, ud->bl, status_skill2sc(TK_RUN), 100,
                                  ud->skill_lv, unit_getdir(ud->bl), 0, 0, 0));
 ...
 */
int unit_set_walkdelay(struct block_list *bl, t_tick tick, t_tick delay,
                       int type) {
  struct unit_data *ud = unit_bl2ud(bl);
  ...
  if (type) {
    ...
    if (bl->type == BL_MOB &&
        status_has_mode(status_get_status_data(bl), MD_STATUSIMMUNE))
      return 0;
    ...
    if (!unit_can_move(bl)) {
      if (bl->type == BL_MOB) {
        ...
        if (md && md->state.alchemist ==
                      1) // Sphere Mine needs to escape, don't stop it
          return 0;
      }
      unit_stop_walking(
          bl, 4); // Unit might still be moving even though it can't move
      return 0;
    }
  ...
  if (ud->walktimer !=
      INVALID_TIMER) { // Stop walking, if chasing, readjust timers.
    if (delay == 1) // Minimal delay (walk-delay) disabled. Just stop walking.
      ...
      if (ud->state.running)
        ...
        if (ud->target)
          add_timer(ud->canmove_tick + 1, unit_walktobl_sub, bl->id,
                    ud->target);
      }
    }
  }
...
/**
 * Performs checks for a unit using a skill and executes after cast time
 * completion
 ...
 * @param castcancel: Whether or not the skill can be cancelled by interruption
 * (hit)
 * @return Success(1); Fail(0);
 */
int unit_skilluse_id2(struct block_list *src, int target_id, uint16 skill_id,
                      uint16 skill_lv, int casttime, int castcancel) {
  struct unit_data *ud;
  ...
  if (sc && sc->data[SC_COMBO] && skill_is_combo(skill_id) &&
    ...
    combo = 1;
  } else if (target_id == src->id && inf & INF_SELF_SKILL &&
  ...
  if (sd) {
    ...
    case MH_TINDER_BREAKER: {
      ...
      if (sc->data[SC_COMBO] &&
          sc->data[SC_COMBO]->val1 == skill_id2) { // It's a combo
        target_id = sc->data[SC_COMBO]->val2;
        combo = 1;
        casttime = -1;
      }
  ...
  // Normally not needed because clif.cpp checks for it, but the at/char/script
  // commands don't! [Skotlex]
  if (ud->skilltimer != INVALID_TIMER && skill_id != SA_CASTCANCEL &&
      skill_id != SO_SPELLFIST)
    return 0;
  ...
  if (skill->inf2[INF2_DISABLENEARNPC] &&
  ...
  if (sd) {
    ...
    case SA_CASTCANCEL:
      ...
      break;
    case BD_ENCORE:
      // Prevent using the dance skill if you no longer have the skill in your
      // tree.
  ...
  if (src->type == BL_NPC) // NPC-objects can override cast distance
    ...
    range = skill_get_range2(src, skill_id, skill_lv,
                             true); // Skill cast distance from database

  // New action request received, delete previous action request if not executed
  // yet
  ...
  if (src->type == BL_PC && ud->walktimer != INVALID_TIMER &&
      !battle_check_range(src, target, range - 1)) {
    ud->stepaction = true;
  ...
  // Check range when not using skill on yourself or is a combo-skill during
  // attack (these are supposed to always have the same range as your attack)
  if (src->id != target_id && (!combo || ud->attacktimer == INVALID_TIMER)) {
    if (skill_get_state(ud->skill_id) == ST_MOVE_ENABLE) {
      ...
      if (!battle_check_range(battle_get_master(src), target, range + 1))
        return 0; // Aegis calc remove trap based on Master position, ignoring
                  // mercenary O.O
  ...
  else if (ud->attacktimer !=
           INVALID_TIMER) // Elsewise, delay current attack sequence
  ...
  case BODY_SLAM:
  case KN_CHARGEATK: {
    unsigned int k = (distance_bl(src, target) - 1) /
                     3; // Range 0-3: 500ms, Range 4-6: 1000ms, Range 7+: 1500ms
    if (k > 2)
      k = 2;
    casttime += casttime * k;
  } break;
#endif
  case GD_EMERGENCYCALL: // Emergency Call double cast when the user has learned
                         // Leap [Daegaladh]
    if (sd &&
        (pc_checkskill(sd, TK_HIGHJUMP) || pc_checkskill(sd, SU_LOPE) >= 3))
      casttime *= 2;
    break;
  case RA_WUGDASH:
  ...
  if (!ud->state.running) // Need TK_RUN or WUGDASH handler to be done before
                          // that, see bugreport:6026
    unit_stop_walking(src, 1); // Even though this is not how official works but
                               // this will do the trick. bugreport:6829
  ...
  clif_skillcasting(src, src->id, target_id, 0, 0, skill_id, skill_lv,
                    skill_get_ele(skill_id, skill_lv), casttime);
  ...
  if (sd && target->type == BL_MOB) {
    ...
    if ((status_has_mode(tstatus, MD_CASTSENSORIDLE) ||
         status_has_mode(tstatus, MD_CASTSENSORCHASE)) &&
  ...
  if (!sd || sd->skillitem != skill_id || skill_get_cast(skill_id, skill_lv))
    ud->canact_tick =
        tick +
        i64max(casttime, max(status_get_amotion(src),
  ...
  if (sc) {
    ...
    if (sc->data[SC_CLOAKING] && !(sc->data[SC_CLOAKING]->val4 & 4) &&
        skill_id != AS_CLOAKING) {
      status_change_end(src, SC_CLOAKING, INVALID_TIMER);

      if (!src->prev)
        return 0; // Warped away!
    } else if (sc->data[SC_CLOAKINGEXCEED] &&
               !(sc->data[SC_CLOAKINGEXCEED]->val4 & 4) &&
               skill_id != GC_CLOAKINGEXCEED) {
 ...
 */
int unit_skilluse_pos(struct block_list *src, short skill_x, short skill_y,
 ...
 * @param skill_lv: Skill Level
 * @param casttime: Initial cast time before cast time reductions
 * @param castcancel: Whether or not the skill can be cancelled by interuption
 * (hit)
 * @return Success(1); Fail(0);
 */
int unit_skilluse_pos2(struct block_list *src, short skill_x, short skill_y,
                       uint16 skill_id, uint16 skill_lv, int casttime,
                       int castcancel) {
  struct map_session_data *sd = NULL;
  struct unit_data *ud = NULL;
  struct status_change *sc;
  ...
  if (ud->skilltimer !=
      INVALID_TIMER) // Normally not needed since clif.cpp checks for it, but
                     // at/char/script commands don't! [Skotlex]
    return 0;
  ...
  if (sd) {
    if (skill_isNotOk(skill_id, sd) ||
        !skill_check_condition_castbegin(sd, skill_id, skill_lv))
      return 0;
    if (skill_id == MG_FIREWALL &&
        !skill_pos_maxcount_check(src, skill_x, skill_y, skill_id, skill_lv,
                                  BL_PC, true))
  ...
  // Fail if the targetted skill is near NPC [Cydh]
  if (skill_get_inf2(skill_id, INF2_DISABLENEARNPC) &&
  ...
  // Remember the skill request from the client while walking to the next cell
  if (src->type == BL_PC && ud->walktimer != INVALID_TIMER &&
      !battle_check_range(src, &bl, range - 1)) {
  ...
  // In official this is triggered even if no cast time.
  clif_skillcasting(src, src->id, 0, skill_x, skill_y, skill_id, skill_lv,
                    skill_get_ele(skill_id, skill_lv), casttime);
...
int unit_set_target(struct unit_data *ud, int target_id) {
  ...
  if (ud->target != target_id) {
    ...
    if (ud->target && (target = map_id2bl(ud->target)) &&
        (ux = unit_bl2ud(target)) && ux->target_count > 0)
      ux->target_count--;

    if (target_id && (target = map_id2bl(target_id)) &&
        (ux = unit_bl2ud(target)) && ux->target_count < 255)
      ux->target_count++;
  }
...
int unit_attack(struct block_list *src, int target_id, int continuous) {
  ...
  if (continuous) // If you're to attack continously, set to auto-chase
                  // character
    ud->chaserange = range;
  ...
  if (DIFF_TICK(ud->attackabletime, gettick()) >
      0) // Do attack next time it is possible. [Skotlex]
    ud->attacktimer =
        add_timer(ud->attackabletime, unit_attack_timer, src->id, 0);
  else // Attack NOW.
 ...
 * @param easy: Easy(1) or Hard(0) path check (hard attempts to go around
 * obstacles)
 ...
 * @param range: The number of cells away from bl that the path should be
 * checked
 * @param easy: Easy(1) or Hard(0) path check (hard attempts to go around
 * obstacles)
 ...
 * @return true or false
 */
bool unit_can_reach_bl(struct block_list *bl, struct block_list *tbl, int range,
                       int easy, short *x, short *y) {
  ...
  if (map_getcell(tbl->m, tbl->x - dx, tbl->y - dy,
                  CELL_CHKNOPASS)) { // Look for a suitable cell to place in.
    int i;

    for (i = 0; i < 8 && map_getcell(tbl->m, tbl->x - dirx[i], tbl->y - diry[i],
                                     CELL_CHKNOPASS);
         i++)
      ;
  ...
  if (!path_search(&wpd, bl->m, bl->x, bl->y, tbl->x - dx, tbl->y - dy, easy,
                   CELL_CHKNOREACH))
    return false;
  ...
  if (bl->type != BL_NPC   // If type is a NPC, please disregard.
      && wpd.path_len > 14 // Official number of walkable cells is 14 if and
                           // only if there is an obstacle between. [malufett]
      && !path_search_long(
             nullptr, bl->m, bl->x, bl->y, tbl->x - dx, tbl->y - dy,
             CELL_CHKNOPASS)) // Check if there is an obstacle between
    return false;
#endif
...
static int unit_attack_timer_sub(struct block_list *src, int tid, t_tick tick) {
  ...
  if (src == NULL || src->prev == NULL || target == NULL ||
      target->prev == NULL)
    return 0;
  ...
  if (!battle_config.sdelay_attack_enable &&
    ...
    // Otherwise, we are in a combo-attack, delay this until your canact time is
    // over. [Skotlex]
    if (ud->state.attack_continue) {
  ...
  if ((unit_is_walking(target) || ud->state.step_attack) &&
      (target->type == BL_PC ||
       !map_getcell(target->m, target->x, target->y, CELL_CHKICEWALL)))
    range++; // Extra range when chasing (does not apply to mobs locked in an
             // icewall)
  ...
  if (DIFF_TICK(ud->attackabletime, tick) <= 0) {
    ...
    if (md) {
      ...
      if (status_has_mode(sstatus, MD_ASSIST) &&
          DIFF_TICK(md->last_linktime, tick) < MIN_MOBLINKTIME) {
        // Link monsters nearby [Skotlex]
        md->last_linktime = tick;
        map_foreachinrange(mob_linksearch, src, md->db->range2, BL_MOB,
                           md->mob_id, target, tick);
      }
    }
...
int unit_skillcastcancel(struct block_list *bl, char type) {
  ...
  if (type & 2) { // See if it can be cancelled.
    ...
    if (sd && (sd->special_state.no_castcancel2 ||
               ((sd->sc.data[SC_UNLIMITEDHUMMINGVOICE] ||
                 sd->special_state.no_castcancel) &&
                !map_flag_gvg2(bl->m) &&
                !map_getmapflag(
                    bl->m, MF_BATTLEGROUND)))) // fixed flags being read the
...
/**
 * Returns the remaining max amount of skill units per object for a specific
 * skill
 * @param ud: Unit data
 ...
 */
int unit_remove_map_(struct block_list *bl, clr_type clrtype, const char *file,
  ...
  if (sc && sc->count) { // map-change/warp dispells.
    ...
    // Ensure the bl is a PC; if so, we'll handle the removal of cloaking and
    // cloaking exceed later
    if (bl->type != BL_PC) {
    ...
    if (sc->data[SC_PROVOKE] && sc->data[SC_PROVOKE]->timer == INVALID_TIMER)
      status_change_end(
          bl, SC_PROVOKE,
          INVALID_TIMER); // End infinite provoke to prevent exploit
    ...
    status_change_end(bl, SC_CURSEDCIRCLE_ATKER,
                      INVALID_TIMER); // callme before warp
  ...
  case BL_PC: {
    ...
    if (map[bl->m].users <= 0 || sd->state.debug_remove_map) {
      // This is only place where map users is decreased, if the mobs were
      // removed too soon then this function was executed too many times
      // [FlavioJS]
      ...
      ShowDebug(
          ...
          " (active=%d connect_new=%d rewarp=%d changemap=%d "
          "debug_remove_map=%d)"
          " from map=%s (users=%d)."
          " Previous call from %s:%d(%s), current call from %s:%d(%s)."
          " Please report this!!!\n",
          sd->status.account_id, sd->status.char_id, sd->state.active,
          sd->state.connect_new, sd->state.rewarp, sd->state.changemap,
          sd->state.debug_remove_map, map[bl->m].name, map[bl->m].users,
          sd->debug_file, sd->debug_line, sd->debug_func, file, line, func);
    } else if (--map[bl->m].users == 0 && battle_config.dynamic_mobs)
      map_removemobs(bl->m);

    if (!pc_isinvisible(
            sd)) // Decrement the number of active pvp players on the map
      --map[bl->m].users_pvp;
  ...
  case BL_PET: {
    ...
    if (pd->pet.intimate <= PET_INTIMATE_NONE &&
        !(pd->master && !pd->master->state.active)) {
  ...
  case BL_HOM: {
    ...
    ud->canact_tick =
        ud->canmove_tick; // It appears HOM do reset the can-act tick.

    if (!hd->homunculus.intimacy &&
        !(hd->master && !hd->master->state.active)) {
      // If logging out, this is deleted on unit_free
  ...
  case BL_MER: {
    ...
    if (mercenary_get_lifetime(md) <= 0 &&
        !(md->master && !md->master->state.active)) {
      clif_clearunit_area(bl, clrtype);
      map_delblock(bl);
      unit_free(bl, CLR_OUTSIGHT);
      map_freeblock_unlock();
  ...
  case BL_ELEM: {
    ...
    if (elemental_get_lifetime(ed) <= 0 &&
        !(ed->master && !ed->master->state.active)) {
      clif_clearunit_area(bl, clrtype);
      map_delblock(bl);
      unit_free(bl, CLR_OUTSIGHT);
      map_freeblock_unlock();
...
void unit_remove_map_pc(struct map_session_data *sd, clr_type clrtype) {
  ...
  // CLR_RESPAWN is the warp from logging out, CLR_TELEPORT is the warp from
  // teleporting, but pets/homunc need to just 'vanish' instead of showing the
  // warping animation.
...
int unit_free(struct block_list *bl, clr_type clrtype) {
  ...
  case BL_PC: {
    ...
    map_foreachpc(clif_friendslist_toggle_sub, sd->status.account_id,
                  sd->status.char_id, 0);
    ...
    if (sd->st &&
        sd->st->state != RUN) { // free attached scripts that are waiting
...
#if PACKETVER_MAIN_NUM >= 20150507 || PACKETVER_RE_NUM >= 20150429 ||          \
  ...
  case BL_MOB: {
    ...
    if (md->guardian_data) {
      ...
      if (md->guardian_data->number >= 0 &&
          md->guardian_data->number < MAX_GUARDIANS)
        gc->guardian[md->guardian_data->number].id = 0;
      else {
        int i;

        ARR_FIND(0, gc->temp_guardians_max, i,
    ...
    if (md->spawn) {
      ...
      if (!md->spawn->state.dynamic) { // Permanently remove the mob
        if (--md->spawn->num == 0) {   // Last freed mob is responsible for
                                       // deallocating the group's spawn data.
          aFree(md->spawn);
          md->spawn = NULL;
        }
      }
