...
int mail_removeitem(struct map_session_data *sd, short flag, int idx,
                    int amount) {
  int i;
  ...
  ARR_FIND(0, MAIL_MAX_ITEM, i,
           sd->mail.item[i].index == idx && sd->mail.item[i].nameid > 0);
...
bool mail_removezeny(struct map_session_data *sd, bool flag) {
  ...
  if (sd->mail.zeny > 0) {
    ...
    if (flag) {
      if (pc_payzeny(sd,
                     sd->mail.zeny +
                         sd->mail.zeny * battle_config.mail_zeny_fee / 100,
                     LOG_TYPE_MAIL, NULL)) {
        return false;
      }
    } else {
 ...
 */
enum mail_attach_result mail_setitem(struct map_session_data *sd, short idx,
                                     uint32 amount) {
  // disable mail
  ...
  if (idx == 0) { // Zeny Transfer
    ...
    if (amount > sd->status.zeny)
      amount =
          sd->status.zeny; // TODO: confirm this behavior for old mail system
#else
    ...
    if (i < MAIL_MAX_ITEM) {
      ...
      if ((amount + sd->mail.item[i].amount) >
      ...
      if (battle_config.mail_attachment_weight) {
        for (j = 0; j < i; j++) {
          total += sd->mail.item[j].amount *
                   (sd->inventory_data[sd->mail.item[j].index]->weight / 10);
        }
    ...
    if (amount > sd->inventory.u.items_inventory[idx].amount)
      return MAIL_ATTACH_ERROR;
    if (!pc_can_give_items(sd) ||
        sd->inventory.u.items_inventory[idx].expire_time ||
        !itemdb_available(sd->inventory.u.items_inventory[idx].nameid) ||
        !itemdb_canmail(&sd->inventory.u.items_inventory[idx],
                        pc_get_group_level(sd)) ||
        (sd->inventory.u.items_inventory[idx].bound &&
         !pc_can_give_bounded_items(sd)))
      return MAIL_ATTACH_UNTRADEABLE;
...
bool mail_setattachment(struct map_session_data *sd, struct mail_message *msg) {
  ...
  for (i = 0, amount = 0; i < MAIL_MAX_ITEM; i++) {
    ...
    if (sd->inventory.u.items_inventory[index].nameid !=
        sd->mail.item[i].nameid)
      return false;
    ...
    if (sd->weight >
        sd->max_weight) // TODO: Why check something weird like this here?
      return false;

    memcpy(&msg->item[i], &sd->inventory.u.items_inventory[index],
           sizeof(struct item));
    msg->item[i].amount = sd->mail.item[i].amount;
  }

  if (sd->mail.zeny < 0 ||
      (sd->mail.zeny + sd->mail.zeny * battle_config.mail_zeny_fee / 100 +
...
void mail_getattachment(struct map_session_data *sd, struct mail_message *msg,
                        int zeny, struct item *item) {
  int i;
  ...
  for (i = 0; i < MAIL_MAX_ITEM; i++) {
    if (item[i].nameid > 0 && item[i].amount > 0) {
      ...
      // If it is a pet egg and the card data does not contain a pet id or other
      // special ids are set
      if (pet != nullptr && item[i].card[0] == 0) {
        ...
        if (pet_create_egg(sd, item[i].nameid)) {
          ...
          // Do not send receive packet so that the mail is still displayed with
          // item attachment
          item_received = false;
        ...
        if (pc_additem(sd, &item[i], item[i].amount, LOG_TYPE_MAIL) ==
            ADDITEM_SUCCESS) {
          item_received = true;
...
void mail_deliveryfail(struct map_session_data *sd, struct mail_message *msg) {
  ...
  if (msg->zeny > 0) {
    pc_getzeny(sd,
               ...
               LOG_TYPE_MAIL, NULL); // Zeny receive (due to failure)
  }
...
bool mail_invalid_operation(struct map_session_data *sd) {
  ...
  if (!map_getmapflag(sd->bl.m, MF_TOWN) &&
      !pc_can_use_command(sd, "mail", COMMAND_ATCOMMAND)) {
    ShowWarning(
        "clif_parse_Mail: char '%s' trying to do invalid mail operations.\n",
        sd->status.name);
    return true;
  }
#endif
 ...
 */
void mail_send(struct map_session_data *sd, const char *dest_name,
               const char *title, const char *body_msg, int body_len) {
  ...
  if (DIFF_TICK(sd->cansendmail_tick, gettick()) > 0) {
    clif_displaymessage(sd->fd,
                        msg_txt(sd, 675)); //"Cannot send mails too fast!!."
  ...
  if (battle_config.mail_daily_count) {
    ...
    // After calling mail_refresh_remaining_amount the status should always be
    // there
    if (sd->sc.data[SC_DAILYSENDMAILCNT] == NULL ||
        sd->sc.data[SC_DAILYSENDMAILCNT]->val2 >=
            battle_config.mail_daily_count) {
      clif_Mail_send(sd, WRITE_MAIL_FAILED_CNT);
      return;
    } else {
      sc_start2(&sd->bl, &sd->bl, SC_DAILYSENDMAILCNT, 100,
                date_get_dayofyear(),
                sd->sc.data[SC_DAILYSENDMAILCNT]->val2 + 1, INFINITE_TICK);
    }
  }
  ...
  if (!mail_setattachment(sd, &msg)) { // Invalid Append condition
    ...
    clif_Mail_send(sd, WRITE_MAIL_FAILED); // fail
    for (i = 0; i < MAIL_MAX_ITEM; i++) {
      mail_removeitem(sd, 0, sd->mail.item[i].index + 2,
                      sd->mail.item[i].amount);
    }
  ...
  if (msg.title[0] == '\0') {
    return; // Message has no length and somehow client verification was
            // skipped.
  }

  if (body_len)
    safestrncpy(msg.body, (char *)body_msg,
                min(body_len + 1, MAIL_BODY_LENGTH));
  else
  ...
  sd->cansendmail_tick =
      gettick() + battle_config.mail_delay; // Flood Protection
}
...
void mail_refresh_remaining_amount(struct map_session_data *sd) {
  ...
  if (sd->sc.data[SC_DAILYSENDMAILCNT] == NULL ||
      sd->sc.data[SC_DAILYSENDMAILCNT]->val1 != doy) {
    sc_start2(&sd->bl, &sd->bl, SC_DAILYSENDMAILCNT, 100, doy, 0,
              INFINITE_TICK);
  }
}
